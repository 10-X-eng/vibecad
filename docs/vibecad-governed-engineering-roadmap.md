...
| G2 — durable Analysis and publication | A plus C/E | **Partial** | Explicit audited v1-to-v2 migration, centralized per-user bounded discovery, latest-attempt recovery evidence, truthful restart classification, append-only host-interruption history, per-analysis quotas, and live publication references are implemented; complete actual provider query/reconnect/collect, phase/crash integration, future migration-chain expansion, governed cross-record cleanup, Native/domain publication wiring, compact document references, corruption/restore acceptance, and installed cross-platform acceptance. |
...
Implemented post-baseline slices include immutable artifact descriptors and admission, content-addressed storage, protected cleanup, per-analysis count/declared-byte quotas, exact live publication references, versioned atomic metadata, an explicit audited and backup-preserving v1-to-v2 migration, centralized per-user application-data ownership, inter-process locking, legal lifecycle transitions, attempt/provider identity, same-analysis retry, artifact retention metadata, additive runtime lifecycle binding, and an independent publication coordinator with exact identity/currentness/authorization/receipt replay checks. Each new attempt stores only bounded inert provider recovery capabilities. Restart classification considers only the latest attempt, requires both provider-declared reconnect support and client-exit survivability plus an external job ID, returns explicit reasons, and can atomically append structured `host_interrupted` evidence for unrecoverable host work. Existing schema-v2 attempts without that additive snapshot remain valid and conservatively non-reconnectable. Bounded read-only global/document discovery enumerates validated current-schema records and selects them only by exact source document UID; corrupt records, legacy records requiring explicit migration, filename/record identity mismatches, malformed capability snapshots, and malformed recovery evidence fail closed instead of disappearing or being silently rewritten. This is a recovery classification and recording foundation, not a real remote reconnect path: no provider query, reattachment, or collection is implemented by this slice. Remaining closure requirements below continue to apply, especially actual provider query/reconnect/collect and confirmation, complete lifecycle-phase and process-crash integration, future migration-chain expansion when needed, governed cross-record cleanup, compact document-reference integration, corruption/restore acceptance, real Native document rebind/transaction wiring, domain migration, and authoritative cross-platform packaging acceptance.
...
6. **Continue G-track closure in dependency order:** next extend the G2 recovery
   foundation through a real provider query/reconnect/collect contract and
   complete phase/crash reconciliation without granting publication authority;
   then complete governed cross-record cleanup, compact references,
