...
| G2 — durable Analysis and publication | A plus C/E | **Partial** | Explicit audited v1-to-v2 migration, centralized per-user bounded discovery, latest-attempt recovery evidence, truthful restart classification, append-only host-interruption history, bounded provider reconnect/status/collect reconciliation, attempt-bound collection receipts, per-analysis quotas, and live publication references are implemented; integrate real provider transport, immutable returned-file admission/hash verification, phase/crash resume, future migration-chain expansion, governed cross-record cleanup, Native/domain publication wiring, compact document references, corruption/restore acceptance, and installed cross-platform acceptance. |
...
Implemented post-baseline slices include immutable artifact descriptors and admission, content-addressed storage, protected cleanup, per-analysis count/declared-byte quotas, exact live publication references, versioned atomic metadata, an explicit audited and backup-preserving v1-to-v2 migration, centralized per-user application-data ownership, inter-process locking, legal lifecycle transitions, attempt/provider identity, same-analysis retry, artifact retention metadata, additive runtime lifecycle binding, and an independent publication coordinator with exact identity/currentness/authorization/receipt replay checks. Each new attempt stores only bounded inert provider recovery capabilities. Restart classification considers only the latest attempt, requires both provider-declared reconnect support and client-exit survivability plus an external job ID, returns explicit reasons, and can atomically append structured `host_interrupted` evidence for unrecoverable host work. A provider-facing recovery coordinator now revalidates the exact live provider identity/capabilities, reconnects only the authorized latest attempt, consumes strict bounded status evidence, and collects only after authoritative provider completion. Generic returned manifest descriptors must match the analysis/provider identity and count/declared-byte bounds; a compact manifest-hash receipt is persisted against the exact attempt, and lifecycle advances only to `collecting`. Transient provider failures preserve durable state, malformed provider evidence fails closed, and this coordinator cannot verify files, mutate CAD, or authorize publication. Existing schema-v2 attempts without the additive snapshot remain valid and conservatively non-reconnectable. Bounded read-only global/document discovery enumerates validated current-schema records and selects them only by exact source document UID; corrupt records, legacy records requiring explicit migration, filename/record identity mismatches, malformed capability snapshots, malformed recovery evidence, and malformed collection receipts fail closed instead of disappearing or being silently rewritten. This remains a fixture-proven recovery foundation, not a production remote route: no real provider credentials/network transport, portable-bundle upload, immutable output download/admission, or file-content hash verification is integrated. Remaining closure requirements below continue to apply, especially real transport/provider integration, complete `collecting`/`verifying` lifecycle-phase and process-crash reconciliation, future migration-chain expansion when needed, governed cross-record cleanup, compact document-reference integration, corruption/restore acceptance, real Native document rebind/transaction wiring, domain migration, and authoritative cross-platform packaging acceptance.
...
6. **Continue G-track closure in dependency order:** next extend the bounded G2
   reconnect/status/collect contract through immutable returned-file transport,
   admission and content-hash verification, then resume `collecting` and
   `verifying` safely across injected crashes without granting publication
   authority. Integrate a real provider only after its credential/network and
   portable-bundle boundaries are proven; then complete governed cross-record
   cleanup, compact references,
   corruption/restore and installed cross-platform acceptance. Continue G5
   acceptance, G7 stale/restart/installed behavior, and the current G8/G9
   Assembly identity/interface/inference acceptance from their authoritative
   owners. Advance X3/X4 and X7-X12 only from the corresponding authoritative
   contracts.
