...
manifests that stored `partdesign_engine` migrate in place; the old accessor is
retained only as a compatibility alias for callers.
...
Native mode receives the active native pack's instructions and bounded state.
It does not receive VibeScript APIs or unrelated workbench contracts.

VibeScript mode receives:

- the active domain contract and actual runtime exports;
- persisted programs for that domain only;
- bounded immutable document-object metadata;
- screenshots and reference images explicitly attached to the conversation;
- recent conversation and enabled intent memory; and
- the exact surface declaration and schemas.

It does not receive native CAD tool descriptions, other workbench APIs, or
other VibeScript domains. `describe_api` is generated by introspecting the
active `DomainAPI`, not from a second hand-written provider schema. Context
debug captures include the resolved engine, domain, surface ID, exact tool
schemas, and active domain context.

## Part Design compatibility

Part Design remains the golden reference implementation. Its seven existing
`vibescript.*` names, provider schemas, source-parametric Body/sketch/feature
API, single-solid BREP validation, parameters, semantic interfaces, stable
publications, downstream invalidation, and asynchronous commit protocol remain
unchanged. A checked-in schema-digest fixture guards that provider contract.

Legacy Part Design artifact directories are not moved. Schema-v1 manifests are
read as schema v2 with `domain=partdesign` and
`workbench=PartDesignWorkbench`.

## Schema-v2 program contract

Every non-Part-Design program persists:
...
Every non-Part-Design domain exposes exactly seven qualified tools:

```text
vibescript.<domain>.describe_api
vibescript.<domain>.inspect_program
...
The complete provider surface contains at most eleven tools after the four
conversation/view capabilities.

The lifecycle is designed around the model's operating loop, not a human-only
editor workflow. At turn start the model receives one bounded domain context
containing the active surface identity, existing programs, live output
identities, and copy-ready domain candidates. It checks for a matching program
before creating one, calls `describe_api` for the actual runtime exports, and
authors against those signatures rather than guessing from general FreeCAD
knowledge. `describe_api` includes a shared machine-readable operating contract
plus the domain-specific units, graph rules, examples, and failure semantics.
...
next revision guard and an `inspect_program` verification call. A failed
candidate returns the failed working revision, whether an accepted live state
was preserved, and the exact inspection/repair loop. `inspect_program` reports
...
|---|---|---|---|
| Part Design | `partdesign` | exact single `solid` publications | Production reference |
...
`perspective` mode. The former `long_helix`, `project_parallel`, and
`project_perspective` names remain non-advertised compatibility wrappers for
persisted source, but `describe_api` emits only the canonical v2 contract.
...
live. Provider context is bounded to 32 sketches and 128 geometry, constraint,
expression, or external-reference entries per sketch.
...
provider context. Text remains structured native Draft text and never accepts
...
The provider context contains a bounded native BIM inventory with exact native,
proxy, Draft and IFC types; placements and editable dimensions; Base, Group,
...
global VibeScript engine in BIM resolves directly to these seven lifecycle tools.
...
The provider context exposes a bounded inventory of native Mesh objects with
stable program identities, point/facet/edge counts, world-space bounds, and a
...
or provider context. Every candidate reauthenticates size and content before
...
Provider context exposes only stable document references, native counts,
bounds, at most eight sample coordinates, attribute presence, compact accepted
...
`inspect_program` exposes bounded accepted evidence so the model can verify the
...
`inspect_program` returns bounded accepted solver, simulation, and exploded-view
...
source occurrence paths. A flexible component is available only when domain
context marks its native Assembly source eligible. The worker reconstructs each
...
provider context, and exact solver/profile diagnostics.
...
exposes accepted evidence through `inspect_program`, retains the authenticated
...
foreign group-member preservation, bounded provider context, and stable
...
edit, input, and reconfiguration updates; and verifies bounded provider context.
...
in-place regeneration, provider context, human properties/expressions and
