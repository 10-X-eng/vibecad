...
if SheetMetalTools.isGuiLoaded():
    ...
    Gui = FreeCAD.Gui
    # Restoring a saved source can import this module on a document worker.
    # Retain the public window reference and GUI classes, but obtain the Qt
    # object on its owner thread, just as command registration does.
    if hasattr(Gui, "runOnMainThread"):
        mw = Gui.runOnMainThread(Gui.getMainWindow)
    else:
