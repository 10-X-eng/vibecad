...
def smUpdateViewObject(obj, update):
    """Apply a short presentation update on the GUI thread, retaining its owner.

    Geometry remains on the recompute thread. Older FreeCAD versions execute
    recompute on the GUI thread and retain their immediate presentation path.
    """
    if not isGuiLoaded():
        return
    ...
    def apply():
        current_document = FreeCAD.listDocuments().get(document_name)
        if current_document is not document:
            return
        current_object = current_document.getObject(object_name)
        if current_object is not obj:
            return
        view = current_object.ViewObject
        if view is not None:
            update(view)

    defer = getattr(FreeCAD.Gui, "deferToNextFrame", None)
    if threading.current_thread() is not threading.main_thread() and callable(defer):
        defer(apply)
    else:
        apply()
...
if isGuiLoaded():
    ...
    def smHideObjects(*args):
        for arg in args:
            if arg:
                smUpdateViewObject(arg, lambda view: setattr(view, "Visibility", False))
