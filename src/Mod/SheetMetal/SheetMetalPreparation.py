...
Explicit preparation observes/publishes on the GUI owner and sends detached
shapes to its worker. Native feature recompute can also restore missing caches
under its existing document ownership. Saved result properties and document
History are never rewritten here.
"""
...
def _observe(sheet):
    ...
    return _observe_inputs(sheet, revision)


def _observe_inputs(sheet, revision=None):
    # Also used by native feature recompute, which already owns document reads.
    # This path must not acquire GUI authority or inspect any view provider.
...
def _publish_caches(observed, results):
    ...
    # Validate the entire chain before publishing. These are transient proxy
    # caches only; persisted shapes, properties and History remain untouched.
...
def prepare_for_recompute(sheet):
    """Restore a clean predecessor's missing cache inside native recompute.

    A downstream sketch edit need not touch a saved upstream sheet, so native
    dependency ordering alone cannot reconstruct its transient mapping. Reuse
    warm ancestors and rebuild only the missing chain, verifying saved hashes
    and current inputs before publishing. Interactive reads remain work-free.
    """
    document = Editable.state_owner(sheet)
    try:
        return Editable.get_state_geometry(sheet)
    except RuntimeError:
        pass
    if not document.Recomputing:
        raise RuntimeError("Sheet recompute preparation requires native recompute ownership")
    observed = _observe_inputs(sheet)
    results = _build(*_payload(observed))
    if not _same(observed, _observe_inputs(sheet)):
        raise RuntimeError("The sheet changed during recompute mapping preparation")
    _publish_caches(observed, results)
    return Editable.get_state_geometry(sheet)
