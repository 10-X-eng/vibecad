...
class SketchExtraction:
    ...
    def edges_to_sketch_object(
        ...
        *,
        ...
        sketch = document.getObject(existing_sketch_name)
        ...
        if FreeCAD.GuiUp:
            ...
            SheetMetalTools.smUpdateViewObject(sketch, update_view)
        return sketch
...
def unfold(
    ...
    """
    return _unfold(shape, root_face_index, bac)


def _unfold(shape, root_face_index, bac, *, region_sink=None):
    # The optional internal sink retains correspondence for the shared-model
    # workflow. Legacy unfold callers keep their signature, result and policy.
    ...
    for e in [
        ...
        try:
            ...
            if region_sink is not None:
                dg.nodes[e[1]]["mapping_alignment"] = alignment_transform
                dg.nodes[e[1]]["mapping_uvref"] = uvref
        except Exception as E:
            if region_sink is not None:
                # An editable map cannot publish an incomplete bend as if it
                # were a planar region. Preserve the legacy warning path below.
                raise RuntimeError(f"Cannot map bend Face{e[1] + 1}") from E
            msg = (
    ...
    for face_id, path in nx.shortest_path(dg, source=root_face_index).items():
        ...
        if region_sink is not None:
            region_sink(
                face_id, shape.Faces[face_id], final_mat, thickness,
                node_data[face_id].get("mapping_alignment"),
                node_data[face_id].get("mapping_uvref"),
            )
...
def getUnfoldSketches(
    ...
    *,
    ...
    if bend_lines and bend_lines.Edges:
        ...
        SheetMetalTools.smUpdateViewObject(
            bend_lines_doc_obj, lambda view: setattr(view, "DrawStyle", "Dashdot"))
    ...
    if bend_labels and bend_labels.Edges:
        ...
        SheetMetalTools.smUpdateViewObject(
            bend_labels_doc_obj, lambda view: setattr(view, "PointSize", 0))
