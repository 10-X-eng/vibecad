...
def getUnfold(k_factor_lookup, solid, facename, kFactorStandard):
    ...
    if TheTree.error_code is not None:
        if TheTree.error_code == 1:
            ...
            document = solid.Document
            document.openTransaction("sanitize")
            sewedShape = sew_Shape(solid)
            solid.Visibility = False
            ob = document.addObject("Part::Feature", "Solid")
            ob.Shape = sewedShape
            ob.Label = solid.Label + "_copy"
            if SheetMetalTools.isGuiLoaded():
                def update_view(view):
                    for prop in ("ShapeColor", "LineColor", "PointColor", "DiffuseColor", "Transparency"):
                        setattr(view, prop, getattr(solid.ViewObject, prop))
                SheetMetalTools.smUpdateViewObject(ob, update_view)
...
def SMmakeSketchfromEdges(edges, name, *, document=None, existingSketch=None):
    precision = 0.1  # Precision in Bspline to BiArcs.
    quasidef = 0.01  # Quasi deflection for Ellipses and Parabola.
    document = document if document is not None else FreeCAD.activeDocument()
    usk = existingSketch if existingSketch is not None else document.addObject(
        "Sketcher::SketchObject", name)
    geo = []
    for e in edges:
...
def getUnfoldSketches(
    ...
    *,
    ...
    unfold_sketch = generateSketch(edges, f"{obj_label}_Sketch", sketchColor,
                                   existingSketches, document=document)
...
def generateSketch(edges, name, color, existingSketches=None, *, document=None):
    ...
    targetSketch = existingSketch if existingSketch is not None else doc.addObject(
        "Sketcher::SketchObject", name)
    try:
        sk = Draft.makeSketch(p.Edges, autoconstraints=True, addTo=targetSketch, delete=False,
                              name=name)
        if sk is None:
            raise RuntimeError("Draft could not convert the Unfold sketch")
        if existingSketch is None:
            sk.Label = name
    except Exception:
        # Only clear the sketch owned by this operation. ActiveObject may be an
        # unrelated feature, including one in another document.
        targetSketch.deleteAllGeometry()
        targetSketch.Placement = FreeCAD.Placement()
        SMLogger.warning(FreeCAD.Qt.translate("Logger", "discretizing Sketch"))
        sk = SMmakeSketchfromEdges(p.Edges, name, document=doc, existingSketch=targetSketch)
    ...
    if FreeCAD.GuiUp:
        ...
        def update_view(view):
            view.LineColor = rgb_color
            view.PointColor = rgb_color
            if hasattr(view, "AutoColor"):
                view.AutoColor = False
        SheetMetalTools.smUpdateViewObject(sk, update_view)
