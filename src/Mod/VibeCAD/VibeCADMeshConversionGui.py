...
def _commit(document: Any, prepared_values: Sequence[Any]) -> Mapping[str, Any]:
    requests = tuple(value.request for value in prepared_values)
    _validate(document, requests)
    document.openTransaction("Convert Mesh to Shape")
    try:
        drafts = []
        outputs = []
...
class _MeshConversionUi:
    def __init__(self, document: Any, requests: Sequence[Any], manager: Any) -> None:
        self.document = document
        self.requests = tuple(requests)
        self.manager = manager
        self.job_id = ""
        self.dialog = QtWidgets.QProgressDialog(
            "Preparing Mesh conversion",
            "Cancel",
            0,
            100,
            Gui.getMainWindow(),
        )
        self.dialog.setWindowTitle("Mesh Conversion")
    ...
    def _finish(
        ...
        if phase == "completed" and result is not None:
            ...
            App.Console.PrintMessage(
                "Mesh conversion completed in the background; the source Mesh remains displayed.\n"
            )
            Gui.getMainWindow().statusBar().showMessage(
                "Mesh conversion completed; source Mesh remains displayed",
                10000,
            )
