...
"""Exact Fixed-joint creation in the human-active Assembly."""
...
def _create_fixed_joint(assembly: Any, joint_group: Any, label: str) -> Any:
    import JointObject
...
def _reject_duplicate_fixed_pair(
    joints: tuple[Any, ...],
    first: Any,
    second: Any,
) -> None:
    ...
    for joint in joints:
        if str(getattr(joint, "JointType", "") or "") != "Fixed":
            continue
        components = _regular_joint_components(joint)
        if components is not None and set(components) == requested:
            raise NativeAssemblyFixedJointError(
                "Those two exact components already have an active Fixed joint."
            )
...
def preflight_fixed_joint(
    ...
    *,
    ...
    """Resolve the complete Fixed-joint request without mutation."""
    ...
    try:
        ...
        raise NativeAssemblyFixedJointError(str(exc)) from exc
    ...
    if first.component is second.component:
        raise NativeAssemblyFixedJointError(
            "A Fixed joint requires two distinct exact Assembly components."
        )
    _reject_duplicate_fixed_pair(
        regular,
        first.component,
        second.component,
    )
    return PreparedFixedJoint(
        assembly=assembly,
...
def apply_fixed_joint(
    ...
    spec: FixedJointSpec,
    *,
    joint_factory: Callable[[Any, Any, str], Any] = _create_fixed_joint,
) -> NativeMutationDraft:
    """Create one real Fixed joint and its exact connector state."""

    prepared = preflight_fixed_joint(document, spec)
    ...
    joint = joint_factory(prepared.assembly, prepared.joint_group, spec.label)
    if (
        joint is None
        or getattr(joint, "Document", None) is not document
        or joint not in list(getattr(prepared.joint_group, "Group", ()) or ())
        or str(getattr(joint, "JointType", "") or "") != "Fixed"
    ):
        raise NativeAssemblyFixedJointError(
            "The native Fixed-joint factory returned the wrong object graph."
        )
    ...
    try:
        ...
        raise NativeAssemblyFixedJointError(
            "The native Fixed-joint connector operation failed."
        ) from exc
...
def _verify_solver_result(
    ...
    if status != 0:
        raise NativeAssemblyFixedJointError(
            "The native solver rejected the new Fixed joint."
        )
    ...
    if joint_result is not None and joint_result.get("status") != "satisfied":
        raise NativeAssemblyFixedJointError(
            "The new Fixed joint is conflicting, redundant, or malformed."
        )
...
def verify_fixed_joint(
    ...
    *,
    ...
    """Prove the exact Fixed-joint graph, connector, solver, and GUI invariants."""
    ...
    if (
        ...
        raise NativeAssemblyFixedJointError(
            "Fixed-joint creation changed Assembly identity, validity, activation, selection, or components."
        )
    regular = active_regular_joints(joint_group)
    grounded = active_grounded_joints(joint_group)
    if (
        ...
        raise NativeAssemblyFixedJointError(
            "Fixed-joint creation changed the wrong active joint objects."
        )
    ...
    if new_objects != (joint,) or after_names != before_names | {str(joint.Name)}:
        raise NativeAssemblyFixedJointError(
            "Fixed-joint creation added objects outside the exact native joint graph."
        )
    if (
        str(getattr(joint, "JointType", "") or "") != "Fixed"
        ...
        raise NativeAssemblyFixedJointError(
            "The created object is not one active native Fixed-joint operation."
        )
    ...
    if set(actual) != set(expected) or any(
        ...
        raise NativeAssemblyFixedJointError(
            "The native Fixed joint did not preserve its exact connectors and offsets."
        )
    ...
    for component, before in value["placements_before"].items():
        if document.getObject(str(component.Name)) is not component:
            raise NativeAssemblyFixedJointError(
                "A movable Assembly component disappeared during Fixed-joint creation."
            )
