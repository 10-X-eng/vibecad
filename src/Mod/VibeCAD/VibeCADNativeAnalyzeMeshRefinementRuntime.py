...
"""Document-bound runtime for FEM mesh refinement resources."""
    ...
    if operation in _CREATE:
        ...
        if "target" not in provided or len(provided) < 2 or not provided <= allowed:
            ...
            raise NativeAnalyzeError(
                "Analyze mesh-refinement arguments do not match the selected operation"
                + (f": {'; '.join(details)}." if details else ".")
            )
        return operation, mode, values
    else:
        raise NativeAnalyzeError("The Analyze mesh-refinement operation is unavailable.")
...
class NativeAnalyzeMeshRefinementRuntime:
        ...
        *,
        ...
        if operation in _CREATE:
            ...
            return run_immediate_mutation(
                context,
                ticket=ticket,
                transaction_name=f"Create FEM Mesh {mode.replace('_', ' ').title()}",
        ...
        return run_immediate_mutation(
            context,
            ticket=ticket,
            transaction_name=f"Edit FEM Mesh {mode.replace('_', ' ').title()}",
