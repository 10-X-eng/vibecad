...
"""Strict parsing and exact-state proof for detached Sketch Chamfer diagnostics."""
...
from VibeCADNativeSketchChamferTarget import LABEL, SketchChamferSpec
...
_FIELDS = frozenset(
    {
        ...
        *_ISSUE_FIELDS,
        ...
        "support_arc_geometry_index",
        "chamfer_geometry_index",
...
class SketchChamferPlan:
    form: str
    input_geometry_indices: tuple[int, int]
    support_arc_geometry_index: int
    chamfer_geometry_index: int
...
def _construction_state(
    ...
    *,
    ...
    source_construction = all(
        bool(json.loads(before_records[index]).get("construction"))
        for index in input_indices
    )
    chamfer_construction = source_construction and corner_index is None
    corner_construction = None if corner_index is None else not source_construction
    return chamfer_construction, corner_construction
...
def _validate_geometry_plan(
    ...
    *,
    ...
    support_arc = json.loads(geometry_records[support_arc_index])
    if (
        ...
        raise NativeSketchError(
            f"{LABEL} feasibility returned the wrong construction support arc."
        )
    chamfer = json.loads(geometry_records[chamfer_index])
    if (
        chamfer.get("kind") != "line"
        or bool(chamfer.get("construction")) is not construction
    ):
        raise NativeSketchError(f"{LABEL} feasibility returned the wrong chamfer line.")

    expected_chamfer_construction, expected_corner_construction = _construction_state(
        before_records,
        input_indices,
        corner_index=corner_index,
    )
    if construction is not expected_chamfer_construction:
        raise NativeSketchError(
            f"{LABEL} feasibility returned incorrect human construction behavior."
        )
    if corner_index is not None:
        corner = json.loads(geometry_records[corner_index])
        if (
            corner.get("kind") != "point"
            or bool(corner.get("construction")) is not expected_corner_construction
        ):
            raise NativeSketchError(
...
def parse_sketch_chamfer_diagnostic(
    result: Any,
    ...
    support_arc_index = _integer(
        result["support_arc_geometry_index"], "support arc index"
    )
    chamfer_index = _integer(result["chamfer_geometry_index"], "chamfer index")
    ...
    if support_arc_index != before_count or geometry_count not in {
        before_count + 2,
        before_count + 3,
    }:
    ...
    if raw_corner is None:
        corner_index = None
        if chamfer_index != before_count + 1 or geometry_count != before_count + 2:
            raise NativeSketchError(
                f"{LABEL} feasibility returned the wrong chamfer index."
            )
    else:
        corner_index = _integer(raw_corner, "corner index")
        if (
            corner_index != before_count + 1
            or chamfer_index != before_count + 2
            or geometry_count != before_count + 3
        ):
            raise NativeSketchError(
                f"{LABEL} feasibility returned the wrong preserved-corner topology."
            )
    ...
    if not math.isclose(
        ...
        raise NativeSketchError(f"{LABEL} feasibility returned a different distance.")
    ...
    _validate_geometry_plan(
        ...
        support_arc_index=support_arc_index,
        chamfer_index=chamfer_index,
        corner_index=corner_index,
        radius_mm=radius_mm,
        construction=construction,
    )
    return SketchChamferPlan(
        form,
        input_indices,
        support_arc_index,
        chamfer_index,
