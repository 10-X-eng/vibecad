...
4. POST `/v1/prompt` `{{"text":"..."}}` to start an in-app Build turn if you
   need the chat loop.
`model.extrude`, `model.revolve`, `model.helix`, and `model.loft` use
`stage=propose` then `stage=apply` on `/v1/native` (propose does not
mutate CAD and returns `preview_id`; apply needs that `preview_id` and
rejects a stale document revision). Sweep, fillet, Boolean, and sketch
still live-commit.
