...
from VibeCADNativeDrawingBroken import PreparedBrokenView, validate_prepared_broken_view
...
DRAWING_BROKEN_PROTOCOL = "vibecad-native-drawing-broken-v1"
...
def create_broken_workspace() -> DrawingBrokenWorkspace:
    temporary = tempfile.TemporaryDirectory(prefix="vibecad-native-drawing-broken-")
    root = Path(temporary.name).resolve()
    try:
        os.chmod(root, 0o700)
        child = freeze_regular_file(
            Path(__file__).with_name("VibeCADNativeDrawingBrokenChild.py").resolve(),
            maximum=1024 * 1024,
        )
