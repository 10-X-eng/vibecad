...
"""Focused real-GUI gate for circular arc helper presentation state."""
...
def _phase(name: str) -> None:
    os.write(2, f"VIBECAD_NATIVE_SKETCH_ARC_OVERLAY_PHASE {name}\n".encode())
...
def _run() -> None:
    ...
    try:
        ...
        document = App.newDocument("NativeSketchArcOverlayGate")
        document.UndoMode = 1
        VibeGui._connect_document_observer()
        sketch = document.addObject("Sketcher::SketchObject", "ArcOverlaySketch")
        sketch.Label = "Native circular arc helper presentation lifecycle"
        document.recompute()
        document.clearUndos()
        process_events(16)
        ...
        assert "Sketcher_ArcOverlay" in surface.command_ids
        ...
        ledger.begin_run("native-sketch-arc-overlay-gui")
        ...
        def native_call(arguments, *, succeeds=True, call_id=None):
            ...
            response = dispatcher.call(
                ...
                call_id or f"arc-overlay-focused-{call_number}",
            )
        ...
        expected = exercise_arc_overlay_case(
            sketch=sketch,
            document=document,
        ...
        process_events(16)
        save_path = Path(tempfile.mkdtemp(prefix="vibecad-native-arc-overlay-")) / (
            "NativeSketchArcOverlay.FCStd"
        )
        ...
        VibeGui._connect_document_observer()
        process_events(16)
        reopened = document.getObject("ArcOverlaySketch")
        assert reopened is not None
        assert Gui.activeDocument().setEdit(reopened.Name)
        process_events(24)
        verify_reopened_arc_overlay(reopened, expected)
        _phase("complete")
        print(
            "VIBECAD_NATIVE_SKETCH_ARC_OVERLAY_GUI_OK "
            "explicit stale no-op human-parity scene-switches read-only reopen",
            flush=True,
        )
        exit_code = 0
    except Exception:
