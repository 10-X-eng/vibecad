...
"""Focused real-GUI transaction and FCStd gate for Sketch Translate."""
...
from vibecad_tests.native_sketch_translate_gui_case import (
    exercise_translate_case,
    verify_reopened_translate,
)
...
def _phase(name: str) -> None:
    os.write(2, f"VIBECAD_NATIVE_SKETCH_TRANSLATE_PHASE {name}\n".encode("ascii"))
...
def _run() -> None:
    ...
    try:
        ...
        document = App.newDocument("NativeSketchTranslateGate")
        document.UndoMode = 1
        VibeGui._connect_document_observer()
        sketch = document.addObject("Sketcher::SketchObject", "TranslateSketch")
        sketch.Label = "Native Translate lifecycle"
        document.recompute()
        document.clearUndos()
        process_events(16)
        ...
        assert "Sketcher_Scale" in surface.command_ids
        ...
        ledger.begin_run("native-sketch-translate-gui")
        ...
        def native_call(arguments, *, succeeds=True, call_id=None):
            ...
            response = dispatcher.call(
                ...
                call_id or f"translate-focused-{call_number}",
            )
        ...
        _phase("translate")
        expected = exercise_translate_case(
            sketch=sketch,
            document=document,
        ...
        process_events(16)
        save_path = Path(tempfile.mkdtemp(prefix="vibecad-native-translate-")) / (
            "NativeSketchTranslate.FCStd"
        )
        ...
        reopened = document.getObject("TranslateSketch")
        assert reopened is not None
        assert Gui.activeDocument().setEdit(reopened.Name)
        process_events(24)
        assert read_active_ribbon_surface(controller).surface_id == "sketch.edit"
        verify_reopened_translate(reopened, expected)
        _phase("complete")
        print(
            "VIBECAD_NATIVE_SKETCH_TRANSLATE_GUI_OK "
            "move array stale identities undo redo reopen selection",
            flush=True,
        )
        exit_code = 0
    except Exception:
