...
"""Focused real-GUI transaction and FCStd lifecycle gate for Sketch Equal."""
...
from vibecad_tests.native_sketch_equal_gui_case import (
    exercise_equal_case,
    verify_reopened_equal,
)
...
def _run() -> None:
    ...
    try:
        ...
        document = App.newDocument("NativeSketchEqualGate")
        document.UndoMode = 1
        VibeGui._connect_document_observer()
        sketch = document.addObject("Sketcher::SketchObject", "EqualSketch")
        sketch.Label = "Native Equal lifecycle"
        ...
        ledger.begin_run("native-sketch-equal-gui")
        ...
        def native_call(arguments, *, succeeds=True, call_id=None):
            ...
            response = dispatcher.call(
                ...
                call_id or f"sketch-equal-call-{call_number}",
            )
            assert response.get("ok") is succeeds, response
            assert edit_boundary(document, sketch, controller) == boundary
            return response

        expected = exercise_equal_case(
            sketch=sketch,
            document=document,
        ...
        process_events(16)
        save_path = Path(tempfile.mkdtemp(prefix="vibecad-native-sketch-equal-")) / (
            "NativeSketchEqual.FCStd"
        )
        ...
        verify_reopened_equal(sketch, expected)
        print(
            "VIBECAD_NATIVE_SKETCH_EQUAL_GUI_OK "
            "families=line,circular,elliptic,hyperbolic,parabolic,bspline_weight "
            "geometry=16 constraints=11",
            flush=True,
        )
        _phase("complete")
        exit_code = 0
