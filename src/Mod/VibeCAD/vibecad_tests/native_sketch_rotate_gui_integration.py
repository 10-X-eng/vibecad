...
"""Focused real-GUI transaction and FCStd gate for Sketch Rotate."""
...
from vibecad_tests.native_sketch_rotate_gui_case import (
    exercise_rotate_case,
    verify_reopened_rotate,
)
...
def _phase(name: str) -> None:
    os.write(2, f"VIBECAD_NATIVE_SKETCH_ROTATE_PHASE {name}\n".encode("ascii"))
...
def _run() -> None:
    ...
    try:
        ...
        document = App.newDocument("NativeSketchRotateGate")
        document.UndoMode = 1
        VibeGui._connect_document_observer()
        sketch = document.addObject("Sketcher::SketchObject", "RotateSketch")
        sketch.Label = "Native Rotate lifecycle"
        document.recompute()
        document.clearUndos()
        process_events(16)
        ...
        ledger.begin_run("native-sketch-rotate-gui")
        ...
        def native_call(arguments, *, succeeds=True, call_id=None):
            ...
            response = dispatcher.call(
                ...
                call_id or f"rotate-focused-{call_number}",
            )
        ...
        process_events(16)
        save_path = Path(tempfile.mkdtemp(prefix="vibecad-native-rotate-")) / (
            "NativeSketchRotate.FCStd"
        )
        ...
        reopened = document.getObject("RotateSketch")
        assert reopened is not None
        assert Gui.activeDocument().setEdit(reopened.Name)
        process_events(24)
        assert read_active_ribbon_surface(controller).surface_id == "sketch.edit"
        verify_reopened_rotate(reopened, expected)
        _phase("complete")
        print(
            "VIBECAD_NATIVE_SKETCH_ROTATE_GUI_OK "
            "move polar_array stale identities undo redo reopen selection",
            flush=True,
        )
        exit_code = 0
    except Exception:
