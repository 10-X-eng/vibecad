...
"""Focused real-GUI lifecycle gate for exact Sketch Join Curves."""
...
from vibecad_tests.native_sketch_join_gui_case import (
    exercise_join_case,
    verify_reopened_join,
)
...
def _phase(name: str) -> None:
    os.write(2, f"VIBECAD_NATIVE_SKETCH_JOIN_PHASE {name}\n".encode())
...
def _run() -> None:
    ...
    try:
        ...
        document = App.newDocument("NativeSketchJoinGate")
        document.UndoMode = 1
        VibeGui._connect_document_observer()
        sketch = document.addObject("Sketcher::SketchObject", "JoinSketch")
        sketch.Label = "Native Join Curves lifecycle"
        document.recompute()
        document.clearUndos()
        process_events(16)
        ...
        assert "Sketcher_JoinCurves" in surface.command_ids
        ...
        ledger.begin_run("native-sketch-join-gui")
        ...
        def native_call(arguments, *, succeeds=True, call_id=None):
            ...
            response = dispatcher.call(
                ...
                call_id or f"join-focused-{call_number}",
            )
        ...
        process_events(16)
        save_path = Path(tempfile.mkdtemp(prefix="vibecad-native-join-")) / (
            "NativeSketchJoin.FCStd"
        )
        ...
        reopened = document.getObject("JoinSketch")
        assert reopened is not None
        assert Gui.activeDocument().setEdit(reopened.Name)
        process_events(24)
        assert read_active_ribbon_surface(controller).surface_id == "sketch.edit"
        verify_reopened_join(reopened, expected)
        _phase("complete")
        print(
            "VIBECAD_NATIVE_SKETCH_JOIN_GUI_OK "
            "c0 identity helpers expression undo redo reopen selection",
            flush=True,
        )
        exit_code = 0
    except Exception:
