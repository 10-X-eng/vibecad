...
"""Focused compiled-GUI gate for durable Sketch internal geometry."""
...
def _phase(name: str) -> None:
    os.write(2, f"VIBECAD_NATIVE_SKETCH_INTERNAL_GEOMETRY_PHASE {name}\n".encode())
...
def _run() -> None:
    ...
    try:
        ...
        document = App.newDocument("NativeSketchInternalGeometryGate")
        document.UndoMode = 1
        VibeGui._connect_document_observer()
        sketch = document.addObject("Sketcher::SketchObject", "InternalGeometrySketch")
        sketch.Label = "Native internal-alignment geometry lifecycle"
        document.recompute()
        process_events(16)
        ...
        assert "Sketcher_RestoreInternalAlignmentGeometry" in surface.command_ids
        ...
        ledger.begin_run("native-sketch-internal-geometry-gui")
        ...
        def native_call(arguments, *, succeeds=True):
            ...
            response = dispatcher.call(
                ...
                f"internal-geometry-focused-{call_number}",
            )
        ...
        _phase("atomic_parity_partial_rollback")
        expected = exercise_internal_alignment_case(
            sketch=sketch,
            document=document,
        ...
        process_events(16)
        save_path = (
            Path(tempfile.mkdtemp(prefix="vibecad-native-internal-geometry-"))
            / "NativeSketchInternalGeometry.FCStd"
        )
        ...
        VibeGui._connect_document_observer()
        process_events(16)
        reopened = document.getObject("InternalGeometrySketch")
        assert reopened is not None
        assert Gui.activeDocument().setEdit(reopened.Name)
        process_events(24)
        verify_reopened_internal_alignment(reopened, expected)
        _phase("complete")
        print(
            "VIBECAD_NATIVE_SKETCH_INTERNAL_GEOMETRY_GUI_OK "
            "five-types atomic stale human-parity partial rollback selection reopen",
            flush=True,
        )
        exit_code = 0
    except Exception:
