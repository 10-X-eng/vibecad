...
"""Focused real-GUI lifecycle gate for exact B-spline knot insertion."""
...
def _phase(name: str) -> None:
    os.write(2, f"VIBECAD_NATIVE_SKETCH_KNOT_INSERTION_PHASE {name}\n".encode())
...
def _run() -> None:
    ...
    try:
        ...
        document = App.newDocument("NativeSketchKnotInsertionGate")
        document.UndoMode = 1
        VibeGui._connect_document_observer()
        sketch = document.addObject(
            "Sketcher::SketchObject", "BSplineKnotInsertionSketch"
        )
        sketch.Label = "Native B-spline knot insertion lifecycle"
        document.recompute()
        document.clearUndos()
        process_events(16)
        ...
        assert "Sketcher_BSplineInsertKnot" in surface.command_ids
        ...
        ledger.begin_run("native-sketch-knot-insertion-gui")
        ...
        def native_call(arguments, *, succeeds=True, call_id=None):
            ...
            response = dispatcher.call(
                ...
                call_id or f"knot-insertion-focused-{call_number}",
            )
        ...
        _phase("insert_bspline_knot")
        expected = exercise_bspline_knot_insertion_case(
            sketch=sketch,
            document=document,
        ...
        process_events(16)
        save_path = Path(tempfile.mkdtemp(prefix="vibecad-native-knot-insert-")) / (
            "NativeSketchKnotInsertion.FCStd"
        )
        ...
        reopened = document.getObject("BSplineKnotInsertionSketch")
        assert reopened is not None
        assert Gui.activeDocument().setEdit(reopened.Name)
        process_events(24)
        assert read_active_ribbon_surface(controller).surface_id == "sketch.edit"
        verify_reopened_bspline_knot_insertion(reopened, expected)
        _phase("complete")
        print(
            "VIBECAD_NATIVE_SKETCH_KNOT_INSERTION_GUI_OK "
