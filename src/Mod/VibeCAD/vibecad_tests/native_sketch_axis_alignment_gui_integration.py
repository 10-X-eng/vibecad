...
"""Focused real-GUI transaction and FCStd gate for Remove Axes Alignment."""
...
def _phase(name: str) -> None:
    os.write(2, f"VIBECAD_NATIVE_SKETCH_AXIS_ALIGNMENT_PHASE {name}\n".encode("ascii"))
...
def _run() -> None:
    ...
    try:
        ...
        document = App.newDocument("NativeSketchAxisAlignmentGate")
        document.UndoMode = 1
        VibeGui._connect_document_observer()
        sketch = document.addObject("Sketcher::SketchObject", "AxisAlignmentSketch")
        sketch.Label = "Native Remove Axes Alignment lifecycle"
        document.recompute()
        document.clearUndos()
        process_events(16)
        ...
        ledger.begin_run("native-sketch-axis-alignment-gui")
        ...
        def native_call(arguments, *, succeeds=True, call_id=None):
            ...
            response = dispatcher.call(
                ...
                call_id or f"axis-alignment-focused-{call_number}",
            )
        ...
        _phase("remove_axis_alignment")
        expected = exercise_axis_alignment_case(
            sketch=sketch,
            document=document,
        ...
        process_events(16)
        save_path = Path(tempfile.mkdtemp(prefix="vibecad-native-axis-alignment-")) / (
            "NativeSketchAxisAlignment.FCStd"
        )
        ...
        reopened = document.getObject("AxisAlignmentSketch")
        assert reopened is not None
        assert Gui.activeDocument().setEdit(reopened.Name)
        process_events(24)
        assert read_active_ribbon_surface(controller).surface_id == "sketch.edit"
        verify_reopened_axis_alignment(reopened, expected)
        _phase("complete")
        print(
            "VIBECAD_NATIVE_SKETCH_AXIS_ALIGNMENT_GUI_OK "
            "horizontal vertical parallel symmetry point distance expression tag "
            "stale noop undo redo reopen selection",
            flush=True,
        )
        exit_code = 0
    except Exception:
