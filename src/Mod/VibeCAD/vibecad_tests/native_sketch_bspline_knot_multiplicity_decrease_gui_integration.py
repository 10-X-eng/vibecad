...
"""Focused real-GUI lifecycle gate for B-spline knot multiplicity decrease."""
...
def _phase(name: str) -> None:
    os.write(
        2,
        f"VIBECAD_NATIVE_SKETCH_KNOT_MULTIPLICITY_DECREASE_PHASE {name}\n".encode(),
    )
...
def _run() -> None:
    ...
    try:
        ...
        document = App.newDocument("NativeSketchKnotMultiplicityDecreaseGate")
        document.UndoMode = 1
        VibeGui._connect_document_observer()
        sketch = document.addObject(
            "Sketcher::SketchObject", "BSplineKnotMultiplicityDecreaseSketch"
        )
        sketch.Label = "Native B-spline knot multiplicity decrease lifecycle"
        document.recompute()
        document.clearUndos()
        process_events(16)
        ...
        assert "Sketcher_BSplineDecreaseKnotMultiplicity" in surface.command_ids
        ...
        ledger.begin_run("native-sketch-knot-multiplicity-decrease-gui")
        ...
        def native_call(arguments, *, succeeds=True, call_id=None):
            ...
            response = dispatcher.call(
                ...
                call_id or f"knot-multiplicity-decrease-focused-{call_number}",
            )
        ...
        _phase("decrease_bspline_knot_multiplicity")
        expected = exercise_bspline_knot_multiplicity_decrease_case(
            sketch=sketch,
            document=document,
        ...
        process_events(16)
        save_path = Path(tempfile.mkdtemp(prefix="vibecad-native-knot-dec-")) / (
            "NativeSketchKnotMultiplicityDecrease.FCStd"
        )
        ...
        reopened = document.getObject("BSplineKnotMultiplicityDecreaseSketch")
        assert reopened is not None
        assert Gui.activeDocument().setEdit(reopened.Name)
        process_events(24)
        assert read_active_ribbon_surface(controller).surface_id == "sketch.edit"
        verify_reopened_bspline_knot_multiplicity_decrease(reopened, expected)
        _phase("complete")
        print(
            "VIBECAD_NATIVE_SKETCH_KNOT_MULTIPLICITY_DECREASE_GUI_OK "
            "loss identity helpers expression undo redo reopen selection",
            flush=True,
        )
        exit_code = 0
    except Exception:
