...
"""Focused real-GUI transaction and FCStd lifecycle gate for Active/Inactive."""
...
from vibecad_tests.native_sketch_active_gui_case import (
    exercise_active_case,
    verify_reopened_active,
)
...
def _run() -> None:
    ...
    try:
        ...
        document = App.newDocument("NativeSketchActiveGate")
        document.UndoMode = 1
        VibeGui._connect_document_observer()
        sketch = document.addObject("Sketcher::SketchObject", "ActiveSketch")
        sketch.Label = "Native Active Inactive lifecycle"
        document.recompute()
        document.clearUndos()
        process_events(16)
        _phase("document")
        ...
        ledger.begin_run("native-sketch-active-gui")
        ...
        def native_call(arguments, *, succeeds=True, call_id=None):
            ...
            response = dispatcher.call(
                ...
                call_id or f"sketch-active-call-{call_number}",
            )
            assert response.get("ok") is succeeds, response
            assert edit_boundary(document, sketch, controller) == boundary
            return response

        expected = exercise_active_case(
            sketch=sketch,
            document=document,
        ...
        process_events(16)
        save_path = (
            Path(tempfile.mkdtemp(prefix="vibecad-native-sketch-active-"))
            / "NativeSketchActive.FCStd"
        )
        ...
        verify_reopened_active(sketch, expected)
        print(
            "VIBECAD_NATIVE_SKETCH_ACTIVE_GUI_OK "
            f"geometry={expected['geometry_count']} "
            f"constraints={expected['constraint_count']} targets=7",
            flush=True,
        )
        _phase("complete")
        exit_code = 0
