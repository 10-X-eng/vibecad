...
"""Focused real-GUI gate for B-spline pole-weight visibility."""
...
def _phase(name: str) -> None:
    os.write(2, f"VIBECAD_NATIVE_SKETCH_BSPLINE_WEIGHT_VIEW_PHASE {name}\n".encode())
...
def _run() -> None:
    ...
    try:
        ...
        document = App.newDocument("NativeSketchBSplineWeightViewGate")
        document.UndoMode = 1
        ...
        sketch.Label = "Native B-spline pole-weight presentation lifecycle"
        document.recompute()
        document.clearUndos()
        process_events(16)
        ...
        assert "Sketcher_BSplinePoleWeight" in surface.command_ids
        ...
        ledger.begin_run("native-sketch-bspline-weight-view-gui")
        ...
        def native_call(arguments, *, succeeds=True, call_id=None):
            ...
            response = dispatcher.call(
                ...
                call_id or f"bspline-weight-view-focused-{call_number}",
            )
        ...
        expected = exercise_bspline_pole_weight_visibility_case(
            sketch=sketch,
            document=document,
        ...
        process_events(16)
        save_path = (
            Path(tempfile.mkdtemp(prefix="vibecad-native-bspline-weight-view-"))
            / "NativeSketchBSplinePoleWeight.FCStd"
        )
        ...
        VibeGui._connect_document_observer()
        process_events(16)
        reopened = document.getObject("BSplinePoleWeightViewSketch")
        assert reopened is not None
        assert Gui.activeDocument().setEdit(reopened.Name)
        process_events(24)
        verify_reopened_bspline_pole_weight_visibility(reopened, expected)
        _phase("complete")
        print(
            "VIBECAD_NATIVE_SKETCH_BSPLINE_WEIGHT_VIEW_GUI_OK "
