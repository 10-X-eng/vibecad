...
"""Real-GUI lifecycle gate for one exact Native Assembly Revolute joint."""
...
def _focused_turn(surface, registry) -> NativeTurnSnapshot:
    ...
    provider_surface = NativeProviderSurface(
        ...
        schemas=(
            ...
            joint_definition.provider_schema(("create_revolute",)),
        ),
...
def _arguments(assembly, components, *, expected_joint_count: int) -> dict:
    return {
        ...
        "label": "Native Base-Arm Revolute",
        "reverse": True,
        "limits": {
            "minimum": {"enabled": True, "degrees": -45.0},
            "maximum": {"enabled": True, "degrees": 120.0},
        },
...
def _run() -> None:
    ...
    try:
        ...
        temporary = tempfile.TemporaryDirectory(
            prefix="vibecad-native-assembly-revolute-joint-"
        )
        path = Path(temporary.name) / "native-assembly-revolute-joint.FCStd"
        document = App.newDocument("NativeAssemblyRevoluteJointGate")
        document.UndoMode = 1
        sources = []
        for index in range(2):
            source = document.addObject("Part::Box", f"RevoluteSource{index + 1}")
            source.Length = 12.0
            source.Width = 10.0
            source.Height = 8.0
        ...
        document.openTransaction("Prepare Revolute-joint fixture")
        components = []
        for index, source in enumerate(sources):
            component = assembly.newObject(
                "App::Link",
                f"RevoluteComponent{index + 1}",
            )
        ...
        assert "Assembly_CreateJointRevolute" in surface.command_ids
        ...
        ledger.begin_run("native-assembly-revolute-joint-gui")
        ...
        invalid = dispatcher.call(
            ...
            "assembly-revolute-stale",
        )
        assert invalid["ok"] is False, invalid
        assert invalid["error_code"] == "NATIVE_ASSEMBLY_REVOLUTE_JOINT_FAILED"
        ...
        assert joint.JointType == "Revolute"
        assert isinstance(joint.Proxy, JointObject.Joint)
        assert isinstance(joint.ViewObject.Proxy, JointObject.ViewProviderJoint)
        assert joint.EnableAngleMin and joint.EnableAngleMax
        assert joint.AngleMin.Value == -45.0
        assert joint.AngleMax.Value == 120.0
        ...
        assert joint.EnableAngleMin and joint.AngleMin.Value == -45.0
        assert joint.EnableAngleMax and joint.AngleMax.Value == 120.0
        ...
        assert summary["joints"][0]["joint_type"] == "Revolute"
        assert summary["joints"][0]["angular_limits"] == {
            "minimum": {"enabled": True, "degrees": -45.0},
            "maximum": {"enabled": True, "degrees": 120.0},
        }
        assert summary["joints"][0]["first"]["offset"]
        assert summary["joints"][0]["second"]["offset"]

        print(
            "VIBECAD_NATIVE_ASSEMBLY_REVOLUTE_JOINT_GUI_OK "
            "components=2 joints=1 limits=true reverse=true transactions=1 reopen=true",
            flush=True,
        )
        exit_code = 0
    except Exception:
