...
"""Focused real-GUI transaction and FCStd gate for Sketch Intersection."""
...
def _phase(name: str) -> None:
    os.write(2, f"VIBECAD_NATIVE_SKETCH_INTERSECTION_PHASE {name}\n".encode("ascii"))
...
def _run() -> None:
    ...
    try:
        ...
        document = App.newDocument("NativeSketchIntersectionGate")
        document.UndoMode = 1
        VibeGui._connect_document_observer()
        source = document.addObject("Part::Feature", "IntersectionSource")
        source.Label = "Exact crossing Intersection source"
        source.Shape = Part.makeLine(
            App.Vector(-12.0, 30.0, -8.0),
            App.Vector(12.0, 30.0, 8.0),
        )
        sketch = document.addObject("Sketcher::SketchObject", "IntersectionSketch")
        sketch.Label = "Native Intersection lifecycle"
        document.recompute()
        document.clearUndos()
        process_events(16)
        ...
        ledger.begin_run("native-sketch-intersection-gui")
        ...
        def native_call(arguments, *, succeeds=True, call_id=None):
            ...
            response = dispatcher.call(
                ...
                call_id or f"intersection-focused-{call_number}",
            )
        ...
        _phase("intersection")
        expected = exercise_intersection_case(
            sketch=sketch,
            document=document,
        ...
        process_events(16)
        save_path = Path(tempfile.mkdtemp(prefix="vibecad-native-intersection-")) / (
            "NativeSketchIntersection.FCStd"
        )
        ...
        reopened = document.getObject("IntersectionSketch")
        assert reopened is not None
        assert Gui.activeDocument().setEdit(reopened.Name)
        process_events(24)
        assert read_active_ribbon_surface(controller).surface_id == "sketch.edit"
        verify_reopened_intersection(reopened, expected)
        _phase("complete")
        print(
            "VIBECAD_NATIVE_SKETCH_INTERSECTION_GUI_OK "
            "intersection stale duplicate undo redo reopen selection",
            flush=True,
        )
        exit_code = 0
    except Exception:
