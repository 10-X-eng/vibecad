...
"""Focused real-GUI transaction and FCStd gate for Geometry-to-B-Spline."""
...
def _phase(name: str) -> None:
    os.write(
        2, f"VIBECAD_NATIVE_SKETCH_NURBS_CONVERSION_PHASE {name}\n".encode("ascii")
    )
...
def _run() -> None:
    ...
    try:
        ...
        document = App.newDocument("NativeSketchNURBSConversionGate")
        document.UndoMode = 1
        VibeGui._connect_document_observer()
        sketch = document.addObject("Sketcher::SketchObject", "NURBSConversionSketch")
        sketch.Label = "Native Geometry to B-Spline lifecycle"
        document.recompute()
        document.clearUndos()
        process_events(16)
        ...
        ledger.begin_run("native-sketch-nurbs-conversion-gui")
        ...
        def native_call(arguments, *, succeeds=True, call_id=None):
            ...
            response = dispatcher.call(
                ...
                call_id or f"nurbs-conversion-focused-{call_number}",
            )
        ...
        _phase("convert_to_nurbs")
        expected = exercise_nurbs_conversion_case(
            sketch=sketch,
            document=document,
        ...
        process_events(16)
        save_path = Path(tempfile.mkdtemp(prefix="vibecad-native-nurbs-")) / (
            "NativeSketchNURBSConversion.FCStd"
        )
        ...
        reopened = document.getObject("NURBSConversionSketch")
        assert reopened is not None
        assert Gui.activeDocument().setEdit(reopened.Name)
        process_events(24)
        assert read_active_ribbon_surface(controller).surface_id == "sketch.edit"
        verify_reopened_nurbs_conversion(reopened, expected)
        _phase("complete")
        print(
            "VIBECAD_NATIVE_SKETCH_NURBS_CONVERSION_GUI_OK "
            "internal external controls knots constraints expression stale point "
