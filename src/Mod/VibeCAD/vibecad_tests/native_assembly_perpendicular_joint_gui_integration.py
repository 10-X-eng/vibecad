...
"""Real-GUI lifecycle gate for one exact Native Assembly Perpendicular joint."""
...
def _focused_turn(surface, registry) -> NativeTurnSnapshot:
    ...
    provider_surface = NativeProviderSurface(
        ...
        schemas=(
            ...
            joint_definition.provider_schema(("create_perpendicular",)),
        ),
...
# Both offsets carry independent translation and yaw, while preserving the
# deliberately parallel initial connector Z axes.
...
def _arguments(assembly, components, *, expected_joint_count: int) -> dict:
    return {
        ...
        "label": "Native Base-Arm Perpendicular",
...
def _run() -> None:
    ...
    try:
        ...
        temporary = tempfile.TemporaryDirectory(
            prefix="vibecad-native-assembly-perpendicular-joint-"
        )
        path = Path(temporary.name) / "native-assembly-perpendicular-joint.FCStd"
        document = App.newDocument("NativeAssemblyPerpendicularJointGate")
        document.UndoMode = 1
        sources = []
        for index in range(2):
            source = document.addObject("Part::Box", f"PerpendicularSource{index + 1}")
            source.Length = 12.0
            source.Width = 10.0
            source.Height = 8.0
        ...
        document.openTransaction("Prepare Perpendicular-joint fixture")
        components = []
        for index, source in enumerate(sources):
            component = assembly.newObject(
                "App::Link", f"PerpendicularComponent{index + 1}"
            )
        ...
        assert "Assembly_CreateJointPerpendicular" in surface.command_ids
        ...
        ledger.begin_run("native-assembly-perpendicular-joint-gui")
        ...
        initial = dispatcher.call(
            "state.read", '{"operation":"active"}', "assembly-perpendicular-state-1"
        )
        ...
        invalid = dispatcher.call(
            ...
            "assembly-perpendicular-stale",
        )
        assert invalid["ok"] is False, invalid
        assert invalid["error_code"] == "NATIVE_ASSEMBLY_PERPENDICULAR_JOINT_FAILED"
        ...
        result = dispatcher.call(
            ASSEMBLY_JOINT_CAPABILITY_NAME,
            encoded,
            "assembly-perpendicular-create",
        )
        assert result["ok"] is True, result
        joint_name = result["joint"]["object_name"]
        assert result["joint_type"] == "Perpendicular"
        ...
        assert joint.JointType == "Perpendicular"
        ...
        assert perpendicular_axes_satisfied(joint)
        ...
        assert joint_summary["joint_type"] == "Perpendicular"
        ...
        print(
            "VIBECAD_NATIVE_ASSEMBLY_PERPENDICULAR_JOINT_GUI_OK "
            "components=2 joints=1 initial_parallel=true "
            "axes_perpendicular=true offsets=true transactions=1 reopen=true",
            flush=True,
        )
        exit_code = 0
    except Exception:
