...
def _prepared(document, context, values):
    return preflight_sketch_radius(
        context,
        prepare_sketch_radius(document.Uid, values),
    )
...
@pytest.mark.parametrize(
    ...
    (
        (_circle(4.0), "circle_radius", 4.0),
        (FakeArc(_circle(5.0), 0.0, math.pi / 2.0), "circular_arc_radius", 5.0),
    ),
)
def test_radius_creates_radius_for_exact_circle_or_circular_arc(
    monkeypatch,
    geometry,
    target_form,
    measured_before,
    ...
    result = verify_sketch_radius(
        document,
        create_sketch_radius(document, prepared),
    )

    assert result["operation"] == "constrain_radius"
...
def test_radius_reference_requires_and_retains_exact_measurement(
    monkeypatch,
    geometry,
) -> None:
    document, sketch, context = _host(monkeypatch, geometry)
...
def test_radius_refuses_stale_reference_measurement(monkeypatch) -> None:
...
def test_radius_refuses_unsupported_exact_target(
    monkeypatch,
    geometry,
    selection,
    message,
...
def test_radius_refuses_bspline_pole_owned_by_group(monkeypatch) -> None:
...
def test_radius_rejects_invalid_exact_contract(monkeypatch, updates) -> None:
    document, _sketch, _context = _host(monkeypatch)
    with pytest.raises(NativeSketchError):
        prepare_sketch_radius(document.Uid, _values(**updates))


def test_radius_refuses_solver_rejection_without_mutation(monkeypatch) -> None:
    document, sketch, context = _host(monkeypatch)
...
def test_radius_refuses_preflight_and_postcondition_drift(monkeypatch) -> None:
    document, sketch, context = _host(monkeypatch)
    prepared = _prepared(document, context, _values())
    sketch.GeometryFacadeList[1].Blocked = True
    with pytest.raises(NativeSketchError, match="after Radius preflight"):
        create_sketch_radius(document, prepared)

    sketch.GeometryFacadeList[1].Blocked = False
    prepared = _prepared(document, context, _values())
    draft = create_sketch_radius(document, prepared)
    sketch.RedundantConstraints.append(0)
    with pytest.raises(NativeSketchError, match="solver conflict or redundancy"):
        verify_sketch_radius(document, draft)
