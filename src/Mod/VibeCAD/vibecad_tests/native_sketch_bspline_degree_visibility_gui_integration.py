...
"""Focused real-GUI gate for B-spline degree-information visibility."""
...
def _phase(name: str) -> None:
    os.write(2, f"VIBECAD_NATIVE_SKETCH_BSPLINE_DEGREE_VIEW_PHASE {name}\n".encode())
...
def _run() -> None:
    ...
    try:
        ...
        document = App.newDocument("NativeSketchBSplineDegreeViewGate")
        document.UndoMode = 1
        VibeGui._connect_document_observer()
        sketch = document.addObject("Sketcher::SketchObject", "BSplineDegreeViewSketch")
        sketch.Label = "Native B-spline degree-information presentation lifecycle"
        document.recompute()
        document.clearUndos()
        process_events(16)
        ...
        assert "Sketcher_BSplineDegree" in surface.command_ids
        ...
        ledger.begin_run("native-sketch-bspline-degree-view-gui")
        ...
        def native_call(arguments, *, succeeds=True, call_id=None):
            ...
            response = dispatcher.call(
                ...
                call_id or f"bspline-degree-view-focused-{call_number}",
            )
        ...
        expected = exercise_bspline_degree_visibility_case(
            sketch=sketch,
            document=document,
        ...
        process_events(16)
        save_path = Path(
            tempfile.mkdtemp(prefix="vibecad-native-bspline-degree-view-")
        ) / ("NativeSketchBSplineDegreeView.FCStd")
        ...
        VibeGui._connect_document_observer()
        process_events(16)
        reopened = document.getObject("BSplineDegreeViewSketch")
        assert reopened is not None
        assert Gui.activeDocument().setEdit(reopened.Name)
        process_events(24)
        verify_reopened_bspline_degree_visibility(reopened, expected)
        _phase("complete")
        print(
            "VIBECAD_NATIVE_SKETCH_BSPLINE_DEGREE_VIEW_GUI_OK "
            "explicit stale no-op human-parity scene-switch read-only reopen",
            flush=True,
        )
        exit_code = 0
    except Exception:
