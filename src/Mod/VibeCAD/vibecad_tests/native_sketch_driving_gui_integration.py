...
"""Focused real-GUI transaction and FCStd lifecycle gate for Driving/Reference."""
...
from vibecad_tests.native_sketch_driving_gui_case import (
    exercise_driving_case,
    verify_reopened_driving,
)
...
def _run() -> None:
    ...
    try:
        ...
        document = App.newDocument("NativeSketchDrivingGate")
        document.UndoMode = 1
        VibeGui._connect_document_observer()
        sketch = document.addObject("Sketcher::SketchObject", "DrivingSketch")
        sketch.Label = "Native Driving Reference lifecycle"
        ...
        ledger.begin_run("native-sketch-driving-gui")
        ...
        def native_call(arguments, *, succeeds=True, call_id=None):
            ...
            response = dispatcher.call(
                ...
                call_id or f"sketch-driving-call-{call_number}",
            )
            assert response.get("ok") is succeeds, response
            assert edit_boundary(document, sketch, controller) == boundary
            return response

        expected = exercise_driving_case(
            sketch=sketch,
            document=document,
        ...
        process_events(16)
        save_path = (
            Path(tempfile.mkdtemp(prefix="vibecad-native-sketch-driving-"))
            / "NativeSketchDriving.FCStd"
        )
        ...
        verify_reopened_driving(sketch, expected)
        print(
            "VIBECAD_NATIVE_SKETCH_DRIVING_GUI_OK "
            f"geometry={expected['geometry_count']} "
            f"constraints={expected['constraint_count']} targets=8",
            flush=True,
        )
        _phase("complete")
        exit_code = 0
