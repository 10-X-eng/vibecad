...
"""Focused real-GUI transaction and FCStd lifecycle gate for Constraint Group."""
...
from vibecad_tests.native_sketch_group_gui_case import (
    exercise_group_case,
    verify_reopened_group,
)
...
def _run() -> None:
    ...
    try:
        ...
        document = App.newDocument("NativeSketchGroupGate")
        document.UndoMode = 1
        VibeGui._connect_document_observer()
        sketch = document.addObject("Sketcher::SketchObject", "GroupSketch")
        sketch.Label = "Native Constraint Group lifecycle"
        ...
        assert SKETCH_CONSTRAINT_CAPABILITY_NAME in production.incomplete_definition_names
        _phase("production")
        ...
        ledger.begin_run("native-sketch-group-gui")
        ...
        def native_call(arguments, *, succeeds=True, call_id=None):
            ...
            response = dispatcher.call(
                ...
                call_id or f"sketch-group-call-{call_number}",
            )
            assert response.get("ok") is succeeds, response
            assert edit_boundary(document, sketch, controller) == boundary
            return response

        expected = exercise_group_case(
            sketch=sketch,
            document=document,
        ...
        process_events(16)
        save_path = (
            Path(tempfile.mkdtemp(prefix="vibecad-native-sketch-group-"))
            / "NativeSketchGroup.FCStd"
        )
        ...
        verify_reopened_group(sketch, expected)
        print(
            "VIBECAD_NATIVE_SKETCH_GROUP_GUI_OK "
            f"geometry={expected['geometry_count']} "
            f"constraints={expected['constraint_count']} "
            f"members={len(expected['member_tags'])} "
            f"deleted_geometry={expected['cleanup']['deleted_geometry_count']} "
            f"deleted_constraints={expected['cleanup']['deleted_constraint_count']}",
            flush=True,
        )
        _phase("complete")
        exit_code = 0
