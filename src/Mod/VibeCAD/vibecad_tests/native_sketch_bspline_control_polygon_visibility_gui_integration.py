...
"""Focused real-GUI gate for B-spline control-polygon visibility."""
...
def _phase(name: str) -> None:
    os.write(2, f"VIBECAD_NATIVE_SKETCH_BSPLINE_POLYGON_PHASE {name}\n".encode())
...
def _run() -> None:
    ...
    try:
        ...
        document = App.newDocument("NativeSketchBSplinePolygonGate")
        document.UndoMode = 1
        ...
        sketch.Label = "Native B-spline control-polygon presentation lifecycle"
        document.recompute()
        document.clearUndos()
        process_events(16)
        ...
        assert "Sketcher_BSplinePolygon" in surface.command_ids
        ...
        ledger.begin_run("native-sketch-bspline-control-polygon-gui")
        ...
        def native_call(arguments, *, succeeds=True, call_id=None):
            ...
            response = dispatcher.call(
                ...
                call_id or f"bspline-control-polygon-focused-{call_number}",
            )
        ...
        expected = exercise_bspline_control_polygon_visibility_case(
            sketch=sketch,
            document=document,
        ...
        process_events(16)
        save_path = (
            Path(tempfile.mkdtemp(prefix="vibecad-native-bspline-polygon-"))
            / "NativeSketchBSplineControlPolygon.FCStd"
        )
        ...
        VibeGui._connect_document_observer()
        process_events(16)
        reopened = document.getObject("BSplineControlPolygonViewSketch")
        assert reopened is not None
        assert Gui.activeDocument().setEdit(reopened.Name)
        process_events(24)
        verify_reopened_bspline_control_polygon_visibility(reopened, expected)
        _phase("complete")
        print(
            "VIBECAD_NATIVE_SKETCH_BSPLINE_POLYGON_GUI_OK "
            "explicit stale no-op human-parity exact-geometry read-only reopen",
            flush=True,
        )
        exit_code = 0
    except Exception:
