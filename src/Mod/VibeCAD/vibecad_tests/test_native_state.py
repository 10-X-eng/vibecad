...
def test_reject_mutation_preview_does_not_apply_and_allows_stale() -> None:
    store = NativeDocumentStateStore()
    store.ensure_document("doc-1")
    preview = store.propose_mutation_preview(
        "doc-1",
    ...
    store.note_structural_change("doc-1")
    rejected = store.reject_mutation_preview("doc-1", preview["preview_id"])
    assert rejected["applied"] is False
    assert rejected["rejected"] is True
    assert rejected["stale"] is True
    assert store.list_mutation_previews("doc-1") == []
    with pytest.raises(NativeStateError, match="NATIVE_PREVIEW_CONSUMED"):
        store.reject_mutation_preview("doc-1", preview["preview_id"])


def test_get_mutation_preview_does_not_consume() -> None:
    store = NativeDocumentStateStore()
    store.ensure_document("doc-1")
    preview = store.propose_mutation_preview(
        "doc-1",
    ...
    fetched = store.get_mutation_preview("doc-1", preview["preview_id"])
    assert fetched["arguments"]["label"] == "Pad"
    assert fetched["applied"] is False
    assert store.list_mutation_previews("doc-1")[0]["preview_id"] == preview["preview_id"]
...
def test_history_delete_features_preview_propose_apply_and_stale() -> None:
    store = NativeDocumentStateStore()
    store.ensure_document("doc-1")
    preview = store.propose_mutation_preview(
        "doc-1",
    ...
    assert stored["operation"] == "delete_features"
...
def test_hole_preview_propose_apply_and_stale() -> None:
    store = NativeDocumentStateStore()
    store.ensure_document("doc-1")
    preview = store.propose_mutation_preview(
        "doc-1",
        capability_name="model.hole",
        arguments={"operation": "hole", "label": "Hole", "stage": "propose"},
    )
    assert preview["applied"] is False
    assert store.current_revision("doc-1") == 0
    stored = store.consume_mutation_preview(
    ...
    assert stored["operation"] == "hole"
    store.ensure_document("doc-2")
    stale = store.propose_mutation_preview(
        "doc-2",
        capability_name="model.hole",
        arguments={"operation": "hole", "label": "Hole2"},
    )
    store.note_structural_change("doc-2")
    with pytest.raises(NativeRevisionConflict):
        store.consume_mutation_preview(
...
def test_transform_pattern_mirror_preview_propose_apply_and_stale() -> None:
    store = NativeDocumentStateStore()
    store.ensure_document("doc-1")
    preview = store.propose_mutation_preview(
        "doc-1",
    ...
    assert stored["operation"] == "pattern"
    assert stored["definition"]["kind"] == "mirror"
    store.ensure_document("doc-2")
    stale = store.propose_mutation_preview(
        "doc-2",
        capability_name="model.transform",
...
def test_transform_pattern_circular_preview_propose_apply_and_stale() -> None:
    store = NativeDocumentStateStore()
    store.ensure_document("doc-1")
    preview = store.propose_mutation_preview(
        "doc-1",
    ...
    assert stored["operation"] == "pattern"
    assert stored["definition"]["kind"] == "circular"
    store.ensure_document("doc-2")
    stale = store.propose_mutation_preview(
        "doc-2",
        capability_name="model.transform",
...
def test_transform_pattern_linear_preview_propose_apply_and_stale() -> None:
    store = NativeDocumentStateStore()
    store.ensure_document("doc-1")
    preview = store.propose_mutation_preview(
        "doc-1",
    ...
    assert stored["operation"] == "pattern"
    assert stored["definition"]["kind"] == "linear"
    store.ensure_document("doc-2")
    stale = store.propose_mutation_preview(
        "doc-2",
        capability_name="model.transform",
...
def test_dressup_thickness_preview_propose_apply_and_stale() -> None:
    ...
    assert stored["operation"] == "thickness"
    store.ensure_document("doc-2")
    stale = store.propose_mutation_preview(
        "doc-2",
        capability_name="model.dressup",
        arguments={"operation": "thickness", "label": "Shell2"},
    )
    store.note_structural_change("doc-2")
    with pytest.raises(NativeRevisionConflict):
        store.consume_mutation_preview(
