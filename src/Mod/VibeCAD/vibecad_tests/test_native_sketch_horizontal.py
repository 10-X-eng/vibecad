...
def _prepared(document, context, values):
    return preflight_sketch_horizontal(
        context,
        prepare_sketch_horizontal(document.Uid, values),
    )


def _apply(document, prepared):
    return verify_sketch_horizontal(
        document,
        create_sketch_horizontal(document, prepared),
    )
...
def test_horizontal_constrains_one_exact_line_without_inference(monkeypatch) -> None:
    document, sketch, context = install_fake_sketch_host(monkeypatch)
    _replace_line(sketch, (2.0, 5.0))

    result = _apply(document, _prepared(document, context, _values()))

    assert result["operation"] == "constrain_horizontal"
    assert result["target_form"] == "line"
    assert result["alignment"] == "horizontal"
...
def test_horizontal_preserves_signed_line_direction(monkeypatch) -> None:
    document, sketch, context = install_fake_sketch_host(monkeypatch)
    _replace_line(sketch, (-2.0, -5.0))

    result = _apply(document, _prepared(document, context, _values()))

    assert result["measured_before"]["delta_x"] == -2.0
    assert result["measured_before"]["delta_y"] == -5.0
    assert result["measured_after"]["delta_x"] == -2.0
    assert result["measured_after"]["delta_y"] == 0.0


def test_horizontal_constrains_one_ordered_point_pair(monkeypatch) -> None:
    document, sketch, context = install_fake_sketch_host(monkeypatch)
    first = sketch.addGeometry(FakePoint(_point(2.0, 3.0)), False)
    second = sketch.addGeometry(FakePoint(_point(2.0, 8.0)), False)
...
def test_horizontal_accepts_origin_as_one_point(monkeypatch) -> None:
    document, sketch, context = install_fake_sketch_host(monkeypatch)
...
def test_horizontal_accepts_external_point_reference(monkeypatch) -> None:
    document, sketch, context = install_fake_sketch_host(monkeypatch)
    point = sketch.addGeometry(FakePoint(_point(2.0, 5.0)), False)
...
def test_horizontal_accepts_curve_center_point(monkeypatch) -> None:
    document, sketch, context = install_fake_sketch_host(monkeypatch)
...
def test_horizontal_refuses_zero_length_line_and_coincident_points(monkeypatch) -> None:
    document, sketch, context = install_fake_sketch_host(monkeypatch)
...
def test_horizontal_refuses_wrong_exact_form(
    monkeypatch,
    selection,
    geometry,
    message,
...
def test_horizontal_refuses_existing_line_axis_or_block_constraint(
    monkeypatch,
    constraint_type,
) -> None:
    document, sketch, context = install_fake_sketch_host(monkeypatch)
...
def test_horizontal_refuses_group_and_internal_geometry(monkeypatch) -> None:
    document, sketch, context = install_fake_sketch_host(monkeypatch)
...
def test_horizontal_refuses_missing_point_lookup(monkeypatch) -> None:
    document, sketch, context = install_fake_sketch_host(monkeypatch)
    _replace_line(sketch, (5.0, 2.0))
...
def test_horizontal_rejects_invalid_exact_contract(monkeypatch, updates) -> None:
    document, _sketch, _context = install_fake_sketch_host(monkeypatch)
    with pytest.raises(NativeSketchError):
        prepare_sketch_horizontal(document.Uid, _values(**updates))


def test_horizontal_refuses_solver_rejection_without_mutation(monkeypatch) -> None:
    document, sketch, context = install_fake_sketch_host(monkeypatch)
    _replace_line(sketch, (5.0, 2.0))
...
def test_horizontal_refuses_feasibility_side_effect(monkeypatch) -> None:
    document, sketch, context = install_fake_sketch_host(monkeypatch)
    _replace_line(sketch, (5.0, 2.0))
...
def test_horizontal_refuses_preflight_and_postcondition_drift(monkeypatch) -> None:
    document, sketch, context = install_fake_sketch_host(monkeypatch)
    _replace_line(sketch, (5.0, 2.0))
    prepared = _prepared(document, context, _values())
    sketch.GeometryFacadeList[0].Blocked = True
    with pytest.raises(NativeSketchError, match="after Horizontal"):
        create_sketch_horizontal(document, prepared)

    sketch.GeometryFacadeList[0].Blocked = False
    prepared = _prepared(document, context, _values())
    monkeypatch.setattr(sketch, "_solve_horizontal_vertical", lambda _constraint: None)
    draft = create_sketch_horizontal(document, prepared)
    with pytest.raises(NativeSketchError, match="does not satisfy"):
        verify_sketch_horizontal(document, draft)
