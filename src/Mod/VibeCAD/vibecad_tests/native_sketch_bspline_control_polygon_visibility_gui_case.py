...
"""Real-GUI parity coverage for B-spline control-polygon visibility."""
...
def _assert_control_polygon_geometry(nodes: tuple[Any, ...]) -> None:
    geometry = polygon_switch_geometry(nodes)
    assert len(geometry) == 1
    assert geometry[0]["vertex_counts"] == (4,)
    points = geometry[0]["points"]
    assert len(points) == 4
    assert tuple((round(point[0], 6), round(point[1], 6)) for point in points) == (
        CUBIC_POLES
    )
    assert all(abs(point[2] - 0.004) <= 1.0e-8 for point in points), points


def exercise_bspline_control_polygon_visibility_case(
    *,
    sketch: Any,
    document: Any,
    ...
    original = preference_snapshot(BSPLINE_CONTROL_POLYGON_PREFERENCE, True)
    try:
        group = App.ParamGet(SKETCH_GENERAL_PREFERENCES)
        group.SetBool(BSPLINE_CONTROL_POLYGON_PREFERENCE, True)
        process_events(16)
        ...
        _assert_control_polygon_geometry(layers["control_polygon"])
        assert switch_states(layers["control_polygon"]) == (coin.SO_SWITCH_ALL,)

        hidden = native_call(
            _arguments(sketch, expected=True, visible=False),
            call_id="rolling-bspline-control-polygon-hide",
        )
        process_events(24)
        ...
        layers = bspline_information_switches(sketch, spline)
        _assert_control_polygon_geometry(layers["control_polygon"])
        assert switch_states(layers["control_polygon"]) == (coin.SO_SWITCH_NONE,)

        stale = native_call(
            _arguments(sketch, expected=True, visible=True),
            succeeds=False,
            call_id="rolling-bspline-control-polygon-stale",
        )
        assert stale["error_code"] == "NATIVE_SKETCH_INVALID", stale
        unchanged = native_call(
            _arguments(sketch, expected=False, visible=False),
            call_id="rolling-bspline-control-polygon-no-op",
        )
        assert unchanged["changed"] is False

        assert Gui.isCommandActive("Sketcher_BSplinePolygon")
        Gui.runCommand("Sketcher_BSplinePolygon")
        process_events(24)
        layers = bspline_information_switches(sketch, spline)
        _assert_control_polygon_geometry(layers["control_polygon"])
        assert switch_states(layers["control_polygon"]) == (coin.SO_SWITCH_ALL,)
        ...
        rehidden = native_call(
            _arguments(sketch, expected=True, visible=False),
            call_id="rolling-bspline-control-polygon-rehide",
        )
        process_events(24)
        ...
        reshown = native_call(
            ...
            call_id="rolling-bspline-control-polygon-reshow",
        )
        process_events(24)
        ...
        return {"state": before, "spline_index": spline, "poles": CUBIC_POLES}
    finally:
        restore_preference(BSPLINE_CONTROL_POLYGON_PREFERENCE, original)
        process_events(16)


def verify_reopened_bspline_control_polygon_visibility(
    sketch: Any,
    ...
    poles = tuple((round(pole.x, 6), round(pole.y, 6)) for pole in geometry.getPoles())
    assert poles == tuple(expected["poles"])
