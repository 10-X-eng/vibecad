...
"""Focused real-GUI transaction and FCStd gate for Sketch Symmetry."""
...
def _phase(name: str) -> None:
    os.write(2, f"VIBECAD_NATIVE_SKETCH_SYMMETRY_PHASE {name}\n".encode("ascii"))
...
def _run() -> None:
    ...
    try:
        ...
        document = App.newDocument("NativeSketchSymmetryGate")
        document.UndoMode = 1
        VibeGui._connect_document_observer()
        sketch = document.addObject("Sketcher::SketchObject", "SymmetrySketch")
        sketch.Label = "Native Symmetry lifecycle"
        document.recompute()
        document.clearUndos()
        process_events(16)
        ...
        ledger.begin_run("native-sketch-symmetry-gui")
        ...
        def native_call(arguments, *, succeeds=True, call_id=None):
            ...
            response = dispatcher.call(
                ...
                call_id or f"symmetry-focused-{call_number}",
            )
        ...
        _phase("symmetry")
        expected = exercise_symmetry_transform_case(
            sketch=sketch,
            document=document,
        ...
        process_events(16)
        save_path = Path(tempfile.mkdtemp(prefix="vibecad-native-symmetry-")) / (
            "NativeSketchSymmetry.FCStd"
        )
        ...
        reopened = document.getObject("SymmetrySketch")
        assert reopened is not None
        assert Gui.activeDocument().setEdit(reopened.Name)
        process_events(24)
        assert read_active_ribbon_surface(controller).surface_id == "sketch.edit"
        verify_reopened_symmetry_transform(reopened, expected)
        _phase("complete")
        print(
            "VIBECAD_NATIVE_SKETCH_SYMMETRY_GUI_OK "
            "line point axis external keep delete constrain stale undo redo reopen selection",
            flush=True,
        )
        exit_code = 0
    except Exception:
