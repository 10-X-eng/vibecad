...
def test_bspline_degree_decrease_schema_is_closed_bounded_and_exact() -> None:
    definition = sketch_geometry_capability_definition()
    schema = definition.provider_schema(("decrease_bspline_degree",))
...
def test_degree_decrease_variant_matches_the_single_target_human_action() -> None:
    definition = sketch_geometry_capability_definition()
    variant = next(
        item
        for item in definition.variants
        if item.operation == "decrease_bspline_degree"
    )
    assert variant.action_ids == frozenset({"Sketcher_BSplineDecreaseDegree"})
    assert variant.surface_ids == frozenset({"sketch.edit"})
    assert variant.exact_target_type == "ActiveSketchExactBSplineAndMaximumDeviation"
