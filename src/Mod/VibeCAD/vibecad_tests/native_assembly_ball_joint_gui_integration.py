...
"""Real-GUI lifecycle gate for one exact Native Assembly Ball joint."""
...
def _focused_turn(surface, registry) -> NativeTurnSnapshot:
    ...
    provider_surface = NativeProviderSurface(
        ...
        schemas=(
            ...
            joint_definition.provider_schema(("create_ball",)),
        ),
...
FIRST_OFFSET = _placement(1.0, -2.0, 3.0, (1.0, 0.0, 0.0), 25.0)
SECOND_OFFSET = _placement(-4.0, 5.0, -6.0, (0.0, 1.0, 0.0), 40.0)
...
def _arguments(assembly, components, *, expected_joint_count: int) -> dict:
    return {
        ...
        "first": _connector(components[0], "Vertex1", FIRST_OFFSET),
        "second": _connector(components[1], "Vertex8", SECOND_OFFSET),
        "label": "Native Base-Arm Ball",
...
def _run() -> None:
    ...
    try:
        ...
        temporary = tempfile.TemporaryDirectory(
            prefix="vibecad-native-assembly-ball-joint-"
        )
        path = Path(temporary.name) / "native-assembly-ball-joint.FCStd"
        document = App.newDocument("NativeAssemblyBallJointGate")
        document.UndoMode = 1
        sources = []
        for index in range(2):
            source = document.addObject("Part::Box", f"BallSource{index + 1}")
            source.Length = 12.0
            source.Width = 10.0
            source.Height = 8.0
        ...
        document.openTransaction("Prepare Ball-joint fixture")
        components = []
        for index, source in enumerate(sources):
            component = assembly.newObject("App::Link", f"BallComponent{index + 1}")
        ...
        assert "Assembly_CreateJointBall" in surface.command_ids
        ...
        ledger.begin_run("native-assembly-ball-joint-gui")
        ...
        assert invalid["error_code"] == "NATIVE_ASSEMBLY_BALL_JOINT_FAILED"
        ...
        result = dispatcher.call(
            ASSEMBLY_JOINT_CAPABILITY_NAME,
            encoded,
            "assembly-ball-create",
        )
        assert result["ok"] is True, result
        joint_name = result["joint"]["object_name"]
        assert result["joint_type"] == "Ball"
        ...
        assert joint_summary["joint_type"] == "Ball"
        ...
        print(
            "VIBECAD_NATIVE_ASSEMBLY_BALL_JOINT_GUI_OK "
            "components=2 joints=1 point_connectors=true offsets=true "
