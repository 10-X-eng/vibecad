...
"""Focused real-GUI lifecycle gate for lossy B-spline degree reduction."""
...
def _phase(name: str) -> None:
    os.write(2, f"VIBECAD_NATIVE_SKETCH_BSPLINE_DECREASE_PHASE {name}\n".encode())
...
def _run() -> None:
    ...
    try:
        ...
        document = App.newDocument("NativeSketchBSplineDegreeDecreaseGate")
        document.UndoMode = 1
        VibeGui._connect_document_observer()
        sketch = document.addObject(
            "Sketcher::SketchObject", "BSplineDegreeDecreaseSketch"
        )
        sketch.Label = "Native B-spline degree reduction lifecycle"
        document.recompute()
        document.clearUndos()
        process_events(16)
        ...
        assert "Sketcher_BSplineDecreaseDegree" in surface.command_ids
        ...
        ledger.begin_run("native-sketch-bspline-degree-decrease-gui")
        ...
        def native_call(arguments, *, succeeds=True, call_id=None):
            ...
            response = dispatcher.call(
                ...
                call_id or f"bspline-degree-decrease-focused-{call_number}",
            )
        ...
        _phase("decrease_bspline_degree")
        expected = exercise_bspline_degree_decrease_case(
            sketch=sketch,
            document=document,
        ...
        process_events(16)
        save_path = Path(tempfile.mkdtemp(prefix="vibecad-native-decrease-")) / (
            "NativeSketchBSplineDegreeDecrease.FCStd"
        )
        ...
        reopened = document.getObject("BSplineDegreeDecreaseSketch")
        assert reopened is not None
        assert Gui.activeDocument().setEdit(reopened.Name)
        process_events(24)
        assert read_active_ribbon_surface(controller).surface_id == "sketch.edit"
        verify_reopened_bspline_degree_decrease(reopened, expected)
        _phase("complete")
        print(
            "VIBECAD_NATIVE_SKETCH_BSPLINE_DEGREE_DECREASE_GUI_OK "
            "loss-limit identity helpers expression undo redo reopen selection",
            flush=True,
        )
        exit_code = 0
    except Exception:
