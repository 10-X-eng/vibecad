...
"""Focused real-GUI transaction and FCStd gate for Sketch Scale."""
...
from vibecad_tests.native_sketch_scale_gui_case import (
    exercise_scale_case,
    verify_reopened_scale,
)
...
def _phase(name: str) -> None:
    os.write(2, f"VIBECAD_NATIVE_SKETCH_SCALE_PHASE {name}\n".encode("ascii"))
...
def _run() -> None:
    ...
    try:
        ...
        document = App.newDocument("NativeSketchScaleGate")
        document.UndoMode = 1
        VibeGui._connect_document_observer()
        sketch = document.addObject("Sketcher::SketchObject", "ScaleSketch")
        sketch.Label = "Native Scale lifecycle"
        document.recompute()
        document.clearUndos()
        process_events(16)
        ...
        assert "Sketcher_Offset" in surface.command_ids
        ...
        ledger.begin_run("native-sketch-scale-gui")
        ...
        def native_call(arguments, *, succeeds=True, call_id=None):
            ...
            response = dispatcher.call(
                ...
                call_id or f"scale-focused-{call_number}",
            )
        ...
        process_events(16)
        save_path = Path(tempfile.mkdtemp(prefix="vibecad-native-scale-")) / (
            "NativeSketchScale.FCStd"
        )
        ...
        reopened = document.getObject("ScaleSketch")
        assert reopened is not None
        assert Gui.activeDocument().setEdit(reopened.Name)
        process_events(24)
        assert read_active_ribbon_surface(controller).surface_id == "sketch.edit"
        verify_reopened_scale(reopened, expected)
        _phase("complete")
        print(
            "VIBECAD_NATIVE_SKETCH_SCALE_GUI_OK "
            "replace copy stale identities undo redo reopen selection",
            flush=True,
        )
        exit_code = 0
    except Exception:
