...
"""Focused real-GUI transaction and FCStd gate for Sketch Offset."""
...
from vibecad_tests.native_sketch_offset_gui_case import (
    exercise_offset_case,
    verify_reopened_offset,
)
...
def _phase(name: str) -> None:
    os.write(2, f"VIBECAD_NATIVE_SKETCH_OFFSET_PHASE {name}\n".encode("ascii"))
...
def _run() -> None:
    ...
    try:
        ...
        document = App.newDocument("NativeSketchOffsetGate")
        document.UndoMode = 1
        VibeGui._connect_document_observer()
        sketch = document.addObject("Sketcher::SketchObject", "OffsetSketch")
        sketch.Label = "Native Offset lifecycle"
        document.recompute()
        document.clearUndos()
        process_events(16)
        ...
        ledger.begin_run("native-sketch-offset-gui")
        ...
        def native_call(arguments, *, succeeds=True, call_id=None):
            ...
            response = dispatcher.call(
                ...
                call_id or f"offset-focused-{call_number}",
            )
        ...
        process_events(16)
        save_path = Path(tempfile.mkdtemp(prefix="vibecad-native-offset-")) / (
            "NativeSketchOffset.FCStd"
        )
        ...
        reopened = document.getObject("OffsetSketch")
        assert reopened is not None
        assert Gui.activeDocument().setEdit(reopened.Name)
        process_events(24)
        assert read_active_ribbon_surface(controller).surface_id == "sketch.edit"
        verify_reopened_offset(reopened, expected)
        _phase("complete")
        print(
            "VIBECAD_NATIVE_SKETCH_OFFSET_GUI_OK "
            "signed arc intersection keep delete constrain stale undo redo reopen selection",
            flush=True,
        )
        exit_code = 0
    except Exception:
