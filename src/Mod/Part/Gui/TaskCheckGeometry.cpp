...
QVector<QString> buildCheckStatusStringVector()
{
    ...
    names.push_back(
        QObject::tr("Invalid point on curve on surface")
    ...
    names.push_back(
        QObject::tr("Invalid curve on closed surface")
...
QVector<QString> buildBOPCheckResultVector()
{
    ...
    results.push_back(
        QObject::tr("Boolean operation: incompatibility of vertex")
    );  // BOPAlgo_IncompatibilityOfVertex
    results.push_back(
        QObject::tr("Boolean operation: incompatibility of edge")
    );  // BOPAlgo_IncompatibilityOfEdge
    results.push_back(
        QObject::tr("Boolean operation: incompatibility of face")
    ...
    results.push_back(
        QObject::tr("Boolean operation: invalid curve on surface")
...
void TaskCheckGeometryResults::setupInterface()
{
    ...
    connect(
        ...
        this,
        &TaskCheckGeometryResults::currentRowChanged
    );
        ...
        if (!shapeCheck.IsValid()) {
            ...
            if (runSignal) {
                ...
                Message_ProgressScope theInnerScope(
                    theScope.Next(),
                    TCollection_AsciiString(label.c_str()),
                    1
                );
                theInnerScope.Show();
...
void TaskCheckGeometryResults::checkSub(
    const BRepCheck_Analyzer& shapeCheck,
    const TopoDS_Shape& shape,
...
void TaskCheckGeometryResults::buildShapeContent(
...
)
{
    ...
    bool advancedShapeContent = App::GetApplication()
                                    .GetUserParameter()
                                    .GetGroup("BaseApp")
                                    ->GetGroup("Preferences")
                                    ->GetGroup("Mod")
                                    ->GetGroup("Part")
                                    ->GetGroup("CheckGeometry")
                                    ->GetBool("AdvancedShapeContent", true);
    ...
    try {
        ...
        }
        {
            ...
            Py::Tuple args(1);
            {
                ...
                Py::Float result(
                    Py::asObject(Part::TopoShape(shape).getPyObject())
                        .callMemberFunction("globalTolerance", args)
                );
...
int TaskCheckGeometryResults::goBOPSingleCheck(
...
)
{
    ...
    // doesn't give us access to it. so I didn't want to run BRepCheck_Analyzer twice to get invalid
    // results.
        ...
        for (; faultyShapes1It.More(); faultyShapes1It.Next()) {
            ...
            if (logErrors) {
                std::clog << faultyEntry->parent->name.toStdString().c_str() << " : "
                          ...
                          << faultyEntry->error.toStdString().c_str() << std::endl;
            }
        }
    }
    return 1;
...
void TaskCheckGeometryResults::dispatchError(ResultEntry* entry, const BRepCheck_Status& stat)
{
    ...
    if (logErrors) {
        std::clog << entry->parent->name.toStdString().c_str() << " : "
                  << entry->name.toStdString().c_str() << " : " << entry->type.toStdString().c_str()
                  << " : " << entry->error.toStdString().c_str() << " (BRepCheck)" << std::endl;
    }
}
...
void TaskCheckGeometryResults::currentRowChanged(const QModelIndex& current, const QModelIndex& previous)
{
    ...
    if (current.isValid()) {
        ...
        if (entry) {
            ...
            for (stringIt = entry->selectionStrings.begin();
                 stringIt != entry->selectionStrings.end();
                 ++stringIt) {
...
void PartGui::goSetupResultTypedSelection(
    ResultEntry* entry,
    const TopoDS_Shape& shape,
    ...
    , resultsBtn(nullptr)
{
    ...
    shapeContentBox = new Gui::TaskView::TaskBox(
        Gui::BitmapFactory().pixmap("Part_CheckGeometry"),
        tr("Shape Content"),
        true,
        nullptr
    ...
    settingsBox = new Gui::TaskView::TaskBox(
        Gui::BitmapFactory().pixmap("Part_CheckGeometry"),
        tr("Settings"),
        true,
        nullptr
    ...
    connect(autoRunCheckBox, &QCheckBox::toggled, this, &TaskCheckGeometryDialog::onAutoRunCheckBoxToggled);
    ...
    runBOPCheckBox->setToolTip(
        tr("Extra boolean operations check that can sometimes find errors that\n"
    ...
    runSingleThreadedCheckBox->setToolTip(
        tr("Run the geometry check in a single thread.  This is slower,\n"
           "but more stable.  Default: false")
    );
    runSingleThreadedCheckBox->setChecked(group->GetBool("RunSingleThreaded", false));
    connect(
        runSingleThreadedCheckBox,
        &QCheckBox::toggled,
        this,
        &TaskCheckGeometryDialog::onRunSingleThreadedCheckBoxToggled
    );
    ...
    connect(
        ...
        &QCheckBox::toggled,
        this,
        &TaskCheckGeometryDialog::onLogErrorsCheckBoxToggled
    );
    ...
    expandShapeContentCheckBox->setToolTip(
        tr("Expand shape content.  Changes will take effect next time you use \n"
           "the check geometry tool.  Default: false")
    );
    expandShapeContentCheckBox->setChecked(group->GetBool("ExpandShapeContent", false));
    connect(
        expandShapeContentCheckBox,
        &QCheckBox::toggled,
        this,
        &TaskCheckGeometryDialog::onExpandShapeContentCheckBoxToggled
    );
    ...
    advancedShapeContentCheckBox->setToolTip(
        tr("Show advanced shape content.  Changes will take effect next time you use \n"
           "the check geometry tool.  Default: false")
    );
    advancedShapeContentCheckBox->setChecked(group->GetBool("AdvancedShapeContent", true));
    connect(
        advancedShapeContentCheckBox,
        &QCheckBox::toggled,
        this,
        &TaskCheckGeometryDialog::onAdvancedShapeContentCheckBoxToggled
    );
    ...
    connect(
        argumentTypeModeCheckBox,
        &QCheckBox::toggled,
        this,
        &TaskCheckGeometryDialog::onArgumentTypeModeCheckBoxToggled
    );
    ...
    connect(
        selfInterModeCheckBox,
        &QCheckBox::toggled,
        this,
        &TaskCheckGeometryDialog::onSelfInterModeCheckBoxToggled
    );
    ...
    connect(
        ...
        &QCheckBox::toggled,
        this,
        &TaskCheckGeometryDialog::onSmallEdgeModeCheckBoxToggled
    );
    ...
    connect(
        rebuildFaceModeCheckBox,
        &QCheckBox::toggled,
        this,
        &TaskCheckGeometryDialog::onRebuildFaceModeCheckBoxToggled
    );
    ...
    connect(
        continuityModeCheckBox,
        &QCheckBox::toggled,
        this,
        &TaskCheckGeometryDialog::onContinuityModeCheckBoxToggled
    );
    ...
    connect(
        ...
        &QCheckBox::toggled,
        this,
        &TaskCheckGeometryDialog::onTangentModeCheckBoxToggled
    );
    ...
    connect(
        mergeVertexModeCheckBox,
        &QCheckBox::toggled,
        this,
        &TaskCheckGeometryDialog::onMergeVertexModeCheckBoxToggled
    );
    ...
    connect(
        mergeEdgeModeCheckBox,
        &QCheckBox::toggled,
        this,
        &TaskCheckGeometryDialog::onMergeEdgeModeCheckBoxToggled
    );
    ...
    connect(
        curveOnSurfaceModeCheckBox,
        &QCheckBox::toggled,
        this,
        &TaskCheckGeometryDialog::onCurveOnSurfaceModeCheckBoxToggled
    );
...
void BOPProgressIndicator::Show(const Message_ProgressScope& theScope, const Standard_Boolean isForce)
{
