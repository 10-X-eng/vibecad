    ...
    , filterSelection(true)
{
    ...
    const auto documentName = document.toLatin1();
    auto modelingSelection =
        PartGui::getModelingSelection(documentName.constData());
...
void Mirroring::findShapes()
{
    ...
    for (auto obj : objs) {
        ...
        if (!shape.isNull()) {
            ...
            child->setData(0, Qt::UserRole, QString::fromLatin1(obj->getNameInDocument()));
            child->setData(0, Qt::UserRole + 1, QVariant::fromValue<qlonglong>(obj->getID()));
            child->setData(
                0,
                Qt::UserRole + 2,
...
bool Mirroring::accept()
{
    ...
    App::Document* activeDoc = App::GetApplication().getDocument(
        (const char*)this->document.toLatin1()
    );
    if (!activeDoc || activeDoc != documentAddress
        || activeDoc->Uid.getValueStr() != documentUid) {
        QMessageBox::critical(
            this,
            windowTitle(),
            tr("The document used to start this mirror task is no longer available.")
        );
        return false;
    }
    ...
    QString label, name, selectionString;
    ...
    if (index == 0) {
        ...
        const auto documentName = document.toLatin1();
        const auto selobjs = PartGui::getModelingSelection(documentName.constData());
        if (selobjs.size() != 1) {
            QMessageBox::critical(
                this,
                windowTitle(),
                tr("Select exactly one plane, planar face, or circular edge as the mirror "
                   "reference.")
            );
            return false;
        }
    ...
    try {
        PartGui::ModelingTaskAttempt attempt(*activeDoc, "Mirroring");

        std::vector<App::DocumentObject*> results;
        results.reserve(items.size());
        for (auto item : items) {
            const long sourceId = item->data(0, Qt::UserRole + 1).toLongLong();
            const auto sourceAddress = reinterpret_cast<App::DocumentObject*>(
                static_cast<quintptr>(
                    item->data(0, Qt::UserRole + 2).toULongLong()
                )
            );
            const auto sourceName =
                item->data(0, Qt::UserRole).toString().toLatin1();
            auto* src = activeDoc->getObjectByID(sourceId);
            if (!src || src != sourceAddress || src->getID() != sourceId
                || !activeDoc->containsObject(src)
                || !src->getNameInDocument()
                || sourceName != src->getNameInDocument()
                || activeDoc->getObject(sourceName.constData()) != src
                || !PartGui::isModelingObjectActive(src)) {
                throw Base::RuntimeError(
                    "Selected mirror source is no longer the active object used to start this task"
                );
            }
            ...
            name = QString::fromLatin1(activeDoc->getUniqueObjectName("Mirroring").c_str());

            const QString factory = QStringLiteral("FreeCAD.getDocument(\"%1\").addObject("
                                                   "\"Part::Mirroring\",\"%2\")")
                                        .arg(this->document, name);
            auto* dst = Gui::Command::runDocumentObjectCommand(
                Gui::Command::App,
                *activeDoc,
                factory.toUtf8(),
                Part::Mirroring::getClassTypeId()
            );
            const QString resultObject = QString::fromStdString(Gui::Command::getObjectCmd(dst));
            const QString sourceObject = QString::fromStdString(Gui::Command::getObjectCmd(src));
            const QString code = QStringLiteral("%1.Source=%2\n"
                                                "%1.Label=u\"%3\"\n"
                                                "%1.Normal=(%4,%5,%6)\n"
                                                "%1.Base=(%7,%8,%9)\n"
                                                "%1.MirrorPlane=(%10)")
                                     .arg(resultObject, sourceObject, label)
                                     .arg(normx)
                                     .arg(normy)
                                     .arg(normz)
                                     .arg(basex)
            ...
            attempt.trackCreatedObject(*dst);
            auto* presentation =
        ...
        QMessageBox::critical(
            this,
            windowTitle(),
            tr("Creating mirror failed.\n%1").arg(QCoreApplication::translate("Exception", error.what()))
        );
        return false;
    }
    catch (...) {
...
{
    const bool accepted = widget->accept();
    if (accepted) {
