...
DrawViewDimension::DrawViewDimension()
{
    ...
    ADD_PROPERTY_TYPE(PrecomputedDimensionVectors,
                      (),
                      cacheGroup,
                      cacheFlags,
                      "Persisted descriptive geometry generated by an isolated worker.");
    ADD_PROPERTY_TYPE(PrecomputedDimensionScalars,
                      (),
                      cacheGroup,
                      cacheFlags,
                      "Persisted radius and area values generated by an isolated worker.");
    ADD_PROPERTY_TYPE(PrecomputedDimensionFlags,
                      (),
                      cacheGroup,
                      cacheFlags,
                      "Persisted descriptive-geometry flags generated by an isolated worker.");
...
std::vector<Base::Vector3d> DrawViewDimension::getPrecomputedDimensionVectors() const
{
    ...
    return {
        ...
        m_linearPoints.second(),
        m_linearPoints.extensionLineFirst(),
        m_linearPoints.extensionLineSecond(),
        m_arrowPositions.first(),
        m_arrowPositions.second(),
        m_anglePoints.first(),
        m_anglePoints.second(),
        m_anglePoints.vertex(),
        angleEnds.extensionLineFirst(),
        angleEnds.extensionLineSecond(),
        m_arcPoints.center,
        m_arcPoints.onCurve.first(),
        m_arcPoints.onCurve.second(),
        m_arcPoints.arcEnds.first(),
        m_arcPoints.arcEnds.second(),
        m_arcPoints.midArc,
        m_areaPoint.center,
    };
}
...
void DrawViewDimension::setPrecomputedDimension(
    const std::vector<Base::Vector3d>& vectors,
    const std::vector<double>& scalars,
    const std::vector<bool>& flags)
{
    constexpr size_t vectorCount = 18;
    constexpr size_t scalarCount = 3;
    constexpr size_t flagCount = 4;
    constexpr double coordinateLimit = 1.0e12;
    if (vectors.size() != vectorCount || scalars.size() != scalarCount
        || flags.size() != flagCount) {
        throw Base::ValueError(
            "Precomputed TechDraw dimension state has the wrong fixed shape");
    }
    for (const Base::Vector3d& vector : vectors) {
        if (!std::isfinite(vector.x) || !std::isfinite(vector.y)
            || !std::isfinite(vector.z) || std::abs(vector.x) > coordinateLimit
            || std::abs(vector.y) > coordinateLimit
            || std::abs(vector.z) > coordinateLimit) {
            throw Base::ValueError(
                "Precomputed TechDraw dimension contains an invalid coordinate");
        }
    }
    for (double scalar : scalars) {
        if (!std::isfinite(scalar) || std::abs(scalar) > coordinateLimit * coordinateLimit) {
            throw Base::ValueError(
    ...
    pointPair arrows(vectors[4], vectors[5]);
    pointPair angleEnds(vectors[6], vectors[7], vectors[9], vectors[10]);
    anglePoints angle;
    angle.ends(angleEnds);
    angle.vertex(vectors[8]);
    arcPoints arc;
    arc.center = vectors[11];
    arc.onCurve = pointPair(vectors[12], vectors[13]);
    arc.arcEnds = pointPair(vectors[14], vectors[15]);
    arc.midArc = vectors[16];
    arc.radius = scalars[0];
    arc.isArc = flags[1];
    arc.arcCW = flags[2];
    areaPoint area;
    area.center = vectors[17];
    area.area = scalars[1];
    area.actualArea = scalars[2];

    boost::dynamic_bitset<> storedFlags(flags.size());
    for (size_t index = 0; index < flags.size(); ++index) {
        storedFlags.set(index, flags[index]);
    }
    PrecomputedDimensionVectors.setValues(vectors);
    PrecomputedDimensionScalars.setValues(scalars);
    PrecomputedDimensionFlags.setValues(storedFlags);

    m_linearPoints = linear;
    m_arrowPositions = arrows;
    m_anglePoints = angle;
    m_arcPoints = arc;
    m_areaPoint = area;
    m_hasGeometry = true;
...
bool DrawViewDimension::restorePrecomputedDimension()
{
    const auto& vectors = PrecomputedDimensionVectors.getValues();
    if (vectors.empty()) {
        return false;
    }
    const auto& storedFlags = PrecomputedDimensionFlags.getValues();
    std::vector<bool> flags(storedFlags.size());
    for (size_t index = 0; index < storedFlags.size(); ++index) {
        flags[index] = storedFlags.test(index);
    }
    setPrecomputedDimension(
        vectors, PrecomputedDimensionScalars.getValues(), flags);
    return true;
}
...
void DrawViewDimension::onDocumentRestored()
{
    ...
    try {
        restorePrecomputedDimension();
    }
    catch (const Base::Exception& error) {
        Base::Console().error(
            "Could not restore precomputed TechDraw dimension for %s: %s\n",
            getNameInDocument(),
            error.what());
        m_hasGeometry = false;
    }
