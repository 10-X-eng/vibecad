...
#include "DrawComplexSectionPy.h"
#include "DrawUtil.h"
...
//NOLINTBEGIN
    ...
    ADD_PROPERTY_TYPE(
        PrecomputedComplexPreparedShape,
        (TopoDS_Shape()),
        "Precomputed Section",
        static_cast<App::PropertyType>(
            App::Prop_Output | App::Prop_ReadOnly | App::Prop_Hidden | App::Prop_NoRecompute),
        "Persisted prepared complex-section shape generated by an isolated worker.");
//NOLINTEND
}

DrawComplexSection::PrecomputedComplexSectionState
DrawComplexSection::getPrecomputedComplexSection() const
{
    if (getTypeId() != DrawComplexSection::getClassTypeId()) {
        throw Base::TypeError(
            "A complex-section cache cannot be read from a derived section type");
    }
    const auto section = getPrecomputedSectionState();
    const TopoDS_Shape& prepared = ProjectionStrategy.getValue() == 0
        ? DrawViewSection::m_preparedShape
        : m_preparedShape;
    if (prepared.IsNull()) {
        throw Base::RuntimeError(
            "The TechDraw complex section has incomplete prepared geometry");
    }
    return {section.cutPieces, section.sectionFaces, prepared, section.centroid};
}

void DrawComplexSection::setPrecomputedComplexSection(
    ...
    const Base::Vector3d& centroid)
{
    if (getTypeId() != DrawComplexSection::getClassTypeId()) {
        throw Base::TypeError(
            "A complex-section cache cannot be applied to a derived section type");
    }
    if (preparedShape.IsNull()) {
        throw Base::ValueError("Precomputed complex-section prepared geometry is empty");
    }
    constexpr std::size_t maxPreparedSolids = 10000;
    constexpr std::size_t maxPreparedFaces = 50000;
    constexpr std::size_t maxPreparedEdges = 200000;
    std::size_t solids = 0;
    std::size_t faces = 0;
    std::size_t edges = 0;
    for (TopExp_Explorer explorer(preparedShape, TopAbs_SOLID); explorer.More(); explorer.Next()) {
        if (++solids > maxPreparedSolids) {
            throw Base::ValueError("Precomputed complex section has too many prepared solids");
        }
    }
    for (TopExp_Explorer explorer(preparedShape, TopAbs_FACE); explorer.More(); explorer.Next()) {
        if (++faces > maxPreparedFaces) {
            throw Base::ValueError("Precomputed complex section has too many prepared faces");
        }
    }
    for (TopExp_Explorer explorer(preparedShape, TopAbs_EDGE); explorer.More(); explorer.Next()) {
        if (++edges > maxPreparedEdges) {
            throw Base::ValueError("Precomputed complex section has too many prepared edges");
        }
    }
    if (faces == 0 || edges == 0) {
        throw Base::ValueError(
            "Precomputed complex section contains no prepared face geometry");
    }

    BRepBuilderAPI_Copy preparedCopy(preparedShape);
    TopoDS_Shape acceptedPrepared = preparedCopy.Shape();
    if (acceptedPrepared.IsNull()) {
        throw Base::ValueError(
            "Precomputed complex-section prepared geometry could not be copied");
    }
    setPrecomputedSectionState(cutPieces, sectionFaces, centroid);
    PrecomputedComplexPreparedShape.setValue(acceptedPrepared, false);
    m_preparedShape = acceptedPrepared;
}
...
TopoDS_Shape DrawComplexSection::makeCuttingTool(double dMax)
{
    ...
    // Save the tool faces for the offset cut-surface intersection. The aligned
    // pass builds its own immutable copy instead of depending on this mutable
    // intermediate. Closed profiles need these side faces as well.
    m_toolFaceShape = makeProfileFaceShape(profileWire, dMax);
...
void DrawComplexSection::makeSectionCut(const TopoDS_Shape& baseShape)
{
    ...
    // DrawViewSection already invokes this virtual method in its background
    // cut job when the GUI is running. A second nested future made completion
    // dependent on a watcher created from the worker thread and never completed
    // in a headless document. Build both independent results in the one existing
    // job so human and isolated-worker execution share deterministic lifecycle
    // semantics without blocking the UI thread.
...
void DrawComplexSection::makeAlignedPieces(const TopoDS_Shape& rawShape)
{
    ...
    // Build the alignment pass from its own bounded profile prism. Depending
    // on m_toolFaceShape made alignment sensitive to the timing and retained
    // state of the preceding cutting-tool pass.
    const TopoDS_Shape toolFaceShape = makeProfileFaceShape(profileWire, m_shapeSize);
    if (toolFaceShape.IsNull()) {
        throw Base::RuntimeError("Complex section failed to create profile faces");
    }
    std::size_t acceptedPieceCount = 0;
...
void DrawComplexSection::postHlrTasks()
{
    DrawViewSection::postHlrTasks();
    // A normal TechDraw recompute supersedes the detached aligned/offset
    // prepared shape just as it supersedes the inherited section cache.
    PrecomputedComplexPreparedShape.setValue(TopoDS_Shape(), false);
}

bool DrawComplexSection::restorePrecomputedComplexSection()
{
    if (getTypeId() != DrawComplexSection::getClassTypeId()) {
        return false;
    }
    ...
    const TopoDS_Shape& prepared = PrecomputedComplexPreparedShape.getValue();
    const std::string storedSourceState = PrecomputedSectionSourceState.getValue();
    if (cutPieces.IsNull() || sectionFaces.IsNull() || prepared.IsNull()
        || storedSourceState.empty()
        || storedSourceState != geometrySourceStateSignature()
        || !geometryMatchesActiveSources()) {
        return false;
    }
    setPrecomputedComplexSection(
        cutPieces,
        sectionFaces,
        prepared,
        PrecomputedSectionCentroid.getValue());
    return true;
}
...
void DrawComplexSection::onDocumentRestored()
{
    ...
    try {
        if (restorePrecomputedComplexSection()) {
            requestPrecomputedSectionPaint();
        }
    }
    catch (const Base::Exception& error) {
        Base::Console().error(
            "Could not restore precomputed TechDraw complex section for %s: %s\n",
            getNameInDocument(),
            error.what());
        ...
        DrawViewSection::m_preparedShape.Nullify();
                                             ...
                                             Base::Vector3d sectionNormal) const
{
    ...
    // Segment directions depend only on the side faces generated by the
    // profile, not on the closed cutting solid or the source-shape extent.
    // Using m_shapeSize here made ordinary repaint depend on transient
    // recompute state and broke restored/precomputed complex sections.
    constexpr double directionPrismDepth = 1.0;
    TopoDS_Shape profileSolidTool =
        makeProfileFaceShape(profileWire, directionPrismDepth);
...
TopoDS_Shape DrawComplexSection::makeProfileFaceShape(
    const TopoDS_Wire& profileWire,
    double dMax) const
{
    if (profileWire.IsNull() || !std::isfinite(dMax) || dMax <= 0.0) {
        throw Base::ValueError("Complex section profile-face inputs are invalid");
    }
    const auto extrudeDir = Base::convertTo<gp_Dir>(getReferenceAxis());
    const gp_Vec extrudeVec = 2.0 * dMax * extrudeDir;
    const TopoDS_Shape prism = BRepPrimAPI_MakePrism(profileWire, extrudeVec).Shape();
    if (prism.IsNull()) {
        throw Base::RuntimeError("Complex section profile-face prism is empty");
    }
    return ShapeUtils::moveShape(
        prism,
        Base::convertTo<Base::Vector3d>(extrudeDir) * -dMax);
}
...
PyObject* DrawComplexSection::getPyObject()
{
    if (PythonObject.is(Py::_None())) {
        PythonObject = Py::Object(new DrawComplexSectionPy(this), true);
    }
