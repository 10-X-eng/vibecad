    ...
    , m_shapeSize(0.0)
{
    ...
    static const char* cacheGroup = "Precomputed Section";
    const auto cacheFlags = static_cast<App::PropertyType>(
        App::Prop_Output | App::Prop_ReadOnly | App::Prop_Hidden | App::Prop_NoRecompute);
    ADD_PROPERTY_TYPE(PrecomputedSectionCutPieces,
                      (TopoDS_Shape()),
                      cacheGroup,
                      cacheFlags,
                      "Persisted section cut generated by an isolated worker.");
    ADD_PROPERTY_TYPE(PrecomputedSectionFaces,
                      (TopoDS_Shape()),
                      cacheGroup,
                      cacheFlags,
                      "Persisted aligned section faces generated by an isolated worker.");
    ADD_PROPERTY_TYPE(PrecomputedSectionCentroid,
                      (0.0, 0.0, 0.0),
                      cacheGroup,
                      cacheFlags,
                      "Source centroid used to generate the persisted section.");
    ADD_PROPERTY_TYPE(PrecomputedSectionSourceState,
                      (""),
                      cacheGroup,
                      cacheFlags,
                      "Timeline, base-view, and source state used to generate the persisted section.");
...
DrawViewSection::PrecomputedSectionState DrawViewSection::getPrecomputedSection() const
{
    if (getTypeId() != DrawViewSection::getClassTypeId()) {
        throw Base::TypeError(
            "A simple-section cache cannot be read from a derived section type");
    }
    return getPrecomputedSectionState();
}

DrawViewSection::PrecomputedSectionState DrawViewSection::getPrecomputedSectionState() const
{
    if (!sectionIntermediateStateIsCurrent() || !geometryMatchesActiveSources()) {
        throw Base::RuntimeError(
            "The TechDraw section has no current completed section geometry");
    }
    if (m_cutPieces.IsNull() || m_sectionTopoDSFaces.IsNull()) {
        throw Base::RuntimeError(
            "The TechDraw section has incomplete section geometry");
    }
    return {m_cutPieces, m_sectionTopoDSFaces, m_saveCentroid};
}

void DrawViewSection::setPrecomputedSection(const TopoDS_Shape& cutPieces,
                                            const TopoDS_Shape& sectionFaces,
                                            const Base::Vector3d& centroid)
{
    if (getTypeId() != DrawViewSection::getClassTypeId()) {
        throw Base::TypeError(
            "A simple-section cache cannot be applied to a derived section type");
    }
    setPrecomputedSectionState(cutPieces, sectionFaces, centroid);
}

void DrawViewSection::setPrecomputedSectionState(const TopoDS_Shape& cutPieces,
                                                 const TopoDS_Shape& sectionFaces,
                                                 const Base::Vector3d& centroid)
{
    if (!isBaseValid() || !geometryMatchesActiveSources()) {
        throw Base::RuntimeError(
            "A precomputed section requires a current base view and projection");
    }
    if (waitingForCut() || waitingForHlr() || waitingForFaces()) {
        throw Base::RuntimeError(
            "A precomputed section cannot replace an in-flight TechDraw result");
    }
    if (cutPieces.IsNull() || sectionFaces.IsNull()
        || !std::isfinite(centroid.x) || !std::isfinite(centroid.y)
        || !std::isfinite(centroid.z)) {
        throw Base::ValueError("Precomputed section geometry is incomplete");
    }

    constexpr std::size_t maxCutSolids = 10000;
    constexpr std::size_t maxCutFaces = 50000;
    constexpr std::size_t maxCutEdges = 200000;
    std::size_t solidCount = 0;
    std::size_t cutFaceCount = 0;
    std::size_t cutEdgeCount = 0;
    for (TopExp_Explorer explorer(cutPieces, TopAbs_SOLID); explorer.More(); explorer.Next()) {
        if (++solidCount > maxCutSolids) {
            throw Base::ValueError("Precomputed section has too many cut solids");
        }
    }
    for (TopExp_Explorer explorer(cutPieces, TopAbs_FACE); explorer.More(); explorer.Next()) {
        if (++cutFaceCount > maxCutFaces) {
            throw Base::ValueError("Precomputed section has too many cut faces");
        }
    }
    for (TopExp_Explorer explorer(cutPieces, TopAbs_EDGE); explorer.More(); explorer.Next()) {
        if (++cutEdgeCount > maxCutEdges) {
            throw Base::ValueError("Precomputed section has too many cut edges");
        }
    }
    if (solidCount == 0 || cutFaceCount == 0 || cutEdgeCount == 0) {
        throw Base::ValueError("Precomputed section cut contains no solid geometry");
    }

    BRep_Builder builder;
    TopoDS_Compound normalizedSectionFaces;
    builder.MakeCompound(normalizedSectionFaces);
    std::size_t sectionFaceCount = 0;
    for (TopExp_Explorer explorer(sectionFaces, TopAbs_FACE); explorer.More(); explorer.Next()) {
        if (++sectionFaceCount > maxCutFaces) {
            throw Base::ValueError("Precomputed section has too many section faces");
        }
        builder.Add(normalizedSectionFaces, explorer.Current());
    }
    if (sectionFaceCount == 0) {
        throw Base::ValueError("Precomputed section contains no cut-surface faces");
    }

    BRepBuilderAPI_Copy cutCopy(cutPieces);
    TopoDS_Shape acceptedCut = cutCopy.Shape();
    if (acceptedCut.IsNull()) {
        throw Base::ValueError("Precomputed section cut could not be copied");
    }
    Base::Vector3d origin(0.0, 0.0, 0.0);
    gp_Ax2 projectionCS = getProjectionCS(origin);
    TopoDS_Shape centeredCut = ShapeUtils::moveShape(acceptedCut, centroid * -1.0);
    TopoDS_Shape preparedCut = ShapeUtils::scaleShape(centeredCut, getScale());
    if (!DrawUtil::fpCompare(Rotation.getValue(), 0.0)) {
        preparedCut = ShapeUtils::rotateShape(preparedCut, projectionCS, Rotation.getValue());
    }
    auto sectionGeometry = makeTDSectionFaces(normalizedSectionFaces);
    if (sectionGeometry.empty()) {
        throw Base::ValueError(
            "Precomputed section cut-surface faces could not be reconstructed");
    }

    const std::string sourceState = geometrySourceStateSignature();
    PrecomputedSectionCutPieces.setValue(acceptedCut, false);
    PrecomputedSectionFaces.setValue(normalizedSectionFaces, false);
    PrecomputedSectionCentroid.setValue(centroid);
    PrecomputedSectionSourceState.setValue(sourceState.c_str());

    m_cutPieces = acceptedCut;
    m_cutShapeRaw = acceptedCut;
    m_cutShape = centeredCut;
    m_preparedShape = preparedCut;
    m_projectionCS = projectionCS;
    m_saveCentroid = centroid;
    m_sectionTopoDSFaces = normalizedSectionFaces;
    m_tdSectionFaces = std::move(sectionGeometry);
    m_lineSets.clear();
    m_cutDependencyState = sourceState;
    m_cutAcceptedState = sourceState;
    waitingForCut(false);
    overrideKeepUpdated(false);
}

void DrawViewSection::requestPrecomputedSectionPaint()
{
    if (!sectionIntermediateStateIsCurrent() || !geometryMatchesActiveSources()) {
        throw Base::RuntimeError(
            "A TechDraw section can only be painted from current completed geometry");
    }
    if (auto* base = getBaseDVP()) {
        base->requestPaint();
    }
    requestPaint();
}

bool DrawViewSection::restorePrecomputedSection()
{
    if (getTypeId() != DrawViewSection::getClassTypeId()) {
        return false;
    }
    const TopoDS_Shape& cutPieces = PrecomputedSectionCutPieces.getValue();
    const TopoDS_Shape& sectionFaces = PrecomputedSectionFaces.getValue();
    const std::string storedSourceState = PrecomputedSectionSourceState.getValue();
    if (cutPieces.IsNull() || sectionFaces.IsNull() || storedSourceState.empty()
        || storedSourceState != geometrySourceStateSignature()
        || !geometryMatchesActiveSources()) {
        return false;
    }
    setPrecomputedSection(
        cutPieces, sectionFaces, PrecomputedSectionCentroid.getValue());
    return true;
}
...
void DrawViewSection::postHlrTasks()
{
    ...
    // A normal TechDraw recompute supersedes any detached section cache. The
    // live intermediate state remains current in memory, while the empty
    // token prevents an older persisted cut from being restored later.
    PrecomputedSectionSourceState.setValue("");
...
void DrawViewSection::unsetupObject()
{
    ...
    if (base) {
        // Adding or removing a section changes the base view's section-line
        // decoration, not its projected source geometry. Repainting preserves
        // an accepted HLR cache and avoids a redundant projection recompute.
        base->requestPaint();
    }
...
void DrawViewSection::onDocumentRestored()
{
    ...
    try {
        if (restorePrecomputedSection()) {
            requestPrecomputedSectionPaint();
        }
    }
    catch (const Base::Exception& error) {
        Base::Console().error(
            "Could not restore precomputed TechDraw section for %s: %s\n",
            getNameInDocument(),
            error.what());
        m_cutAcceptedState.clear();
        m_cutDependencyState.clear();
        m_cutPieces.Nullify();
        m_cutShape.Nullify();
