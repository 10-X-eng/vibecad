...
DrawViewDetail::DrawViewDetail() : m_waitingForDetail(false), m_saveDvp(nullptr), m_saveDvs(nullptr)
{
    ...
    static const char* cacheGroup = "Precomputed Detail";
    const auto cacheFlags = static_cast<App::PropertyType>(
        App::Prop_Output | App::Prop_ReadOnly | App::Prop_Hidden | App::Prop_NoRecompute);
    ADD_PROPERTY_TYPE(PrecomputedDetailShape,
                      (TopoDS_Shape()),
                      cacheGroup,
                      cacheFlags,
                      "Persisted detail cut generated by an isolated worker.");
    ADD_PROPERTY_TYPE(PrecomputedDetailSourceState,
                      (""),
                      cacheGroup,
                      cacheFlags,
                      "Timeline, base-view, and source state used to generate the persisted detail.");
...
DrawViewDetail::PrecomputedDetailState DrawViewDetail::getPrecomputedDetail() const
{
    if (!detailIntermediateStateIsCurrent() || m_detailShape.IsNull()) {
        const_cast<DrawViewDetail*>(this)->restorePrecomputedDetail();
    }
    if (!detailIntermediateStateIsCurrent() || m_detailShape.IsNull()) {
        throw Base::RuntimeError(
            "The TechDraw detail has no current completed detail geometry");
    }
    return {m_detailShape};
}

void DrawViewDetail::setPrecomputedDetail(const TopoDS_Shape& detailShape)
{
    adoptPrecomputedDetail(detailShape, true);
}

void DrawViewDetail::adoptPrecomputedDetail(const TopoDS_Shape& detailShape, bool persist)
{
    auto* base = freecad_cast<DrawViewPart*>(BaseView.getValue());
    if (!base || base->getDocument() != getDocument() || !geometryMatchesActiveSources()) {
        throw Base::RuntimeError(
            "A precomputed detail requires a current same-document base view and projection");
    }
    if (waitingForDetail() || waitingForHlr() || waitingForFaces()) {
        throw Base::RuntimeError(
            "A precomputed detail cannot replace an in-flight TechDraw result");
    }
    if (detailShape.IsNull()) {
        throw Base::ValueError("Precomputed detail geometry is empty");
    }

    constexpr std::size_t maxDetailSolids = 10000;
    constexpr std::size_t maxDetailFaces = 50000;
    constexpr std::size_t maxDetailEdges = 200000;
    std::size_t solidCount = 0;
    std::size_t faceCount = 0;
    std::size_t edgeCount = 0;
    for (TopExp_Explorer explorer(detailShape, TopAbs_SOLID); explorer.More(); explorer.Next()) {
        if (++solidCount > maxDetailSolids) {
            throw Base::ValueError("Precomputed detail has too many solids");
        }
    }
    for (TopExp_Explorer explorer(detailShape, TopAbs_FACE); explorer.More(); explorer.Next()) {
        if (++faceCount > maxDetailFaces) {
            throw Base::ValueError("Precomputed detail has too many faces");
        }
    }
    for (TopExp_Explorer explorer(detailShape, TopAbs_EDGE); explorer.More(); explorer.Next()) {
        if (++edgeCount > maxDetailEdges) {
            throw Base::ValueError("Precomputed detail has too many edges");
        }
    }
    if (edgeCount == 0) {
        throw Base::ValueError("Precomputed detail contains no usable geometry");
    }

    BRepBuilderAPI_Copy copy(detailShape);
    TopoDS_Shape accepted = copy.Shape();
    if (accepted.IsNull()) {
        throw Base::ValueError("Precomputed detail geometry could not be copied");
    }

    const std::string sourceState = geometrySourceStateSignature();
    if (persist) {
        PrecomputedDetailShape.setValue(accepted, false);
        PrecomputedDetailSourceState.setValue(sourceState.c_str());
    }
    m_detailShape = accepted;
    m_detailDependencyState = sourceState;
    m_detailAcceptedState = sourceState;
    waitingForDetail(false);
    overrideKeepUpdated(false);
}

void DrawViewDetail::requestPrecomputedDetailPaint()
{
    if (!detailIntermediateStateIsCurrent() || !geometryMatchesActiveSources()) {
        throw Base::RuntimeError(
            "A TechDraw detail can only be painted from current completed geometry");
    }
    if (auto* base = freecad_cast<DrawViewPart*>(BaseView.getValue())) {
        base->requestPaint();
    }
    requestPaint();
}

bool DrawViewDetail::restorePrecomputedDetail()
{
    const TopoDS_Shape& detailShape = PrecomputedDetailShape.getValue();
    const std::string storedSourceState = PrecomputedDetailSourceState.getValue();
    if (detailShape.IsNull() || storedSourceState.empty()
        || storedSourceState != geometrySourceStateSignature()
        || !geometryMatchesActiveSources()) {
        return false;
    }
    adoptPrecomputedDetail(detailShape, false);
    return true;
}
...
void DrawViewDetail::onChanged(const App::Property* prop)
{
    ...
    if (prop == &BaseView || prop == &AnchorPoint || prop == &Radius
        || prop == &Source || prop == &Direction || prop == &XDirection
        || prop == &Rotation || prop == &Scale || prop == &ScaleType) {
...
void DrawViewDetail::onUndoRedoFinished()
{
    DrawViewPart::onUndoRedoFinished();
    try {
        if (restorePrecomputedDetail()) {
            requestPrecomputedDetailPaint();
        }
    }
    catch (const Base::Exception& error) {
        Base::Console().error(
            "Could not restore precomputed TechDraw detail after undo/redo for %s: %s\n",
            getNameInDocument(),
            error.what());
        m_detailAcceptedState.clear();
        m_detailDependencyState.clear();
        m_detailShape.Nullify();
    }
}
...
void DrawViewDetail::postHlrTasks(void)
{
    ...
    // A normal recompute supersedes any detached detail cache.
    PrecomputedDetailSourceState.setValue("");
...
bool DrawViewDetail::detailIntermediateStateIsCurrent() const
{
    return isActiveInDocumentTimeline()
        && !m_detailAcceptedState.empty()
...
TopoDS_Shape DrawViewDetail::getDetailShape() const
{
    ...
    if (!detailIntermediateStateIsCurrent()) {
        return {};
    }
    return m_detailShape;
}

void DrawViewDetail::onDocumentRestored()
{
    ...
    try {
        ...
        Base::Console().error(
            "Could not restore precomputed TechDraw detail for %s: %s\n",
            getNameInDocument(),
            error.what());
        m_detailAcceptedState.clear();
        m_detailDependencyState.clear();
...
PyObject* DrawViewDetail::getPyObject()
{
    if (PythonObject.is(Py::_None())) {
        PythonObject = Py::Object(new DrawViewDetailPy(this), true);
    }
