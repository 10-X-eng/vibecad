...
class TestVibeCADRibbonTools(unittest.TestCase):
    ...
    def test_mesh_repair_service_creates_one_native_history_operation(self):
        from tool_impl.service import mesh_repair
        ...
        source.Mesh = _tetrahedron_with_duplicate_facet()
        self.document.recompute()
        self.assertEqual(source.Mesh.CountFacets, 5)
        objects_before = tuple(self.document.Objects)
        undo_before = self.document.UndoCount

        response = mesh_repair.run(
            Service(self.document),
            object_name=source.Name,
            harmonize_normals=False,
            remove_duplicates=True,
            remove_non_manifolds=False,
            fix_degenerations=False,
            fix_self_intersections=False,
            fill_holes_max_edges=0,
        )

        self.assertTrue(response["ok"], response)
        result = self.document.getObject(response["object"])
        self.assertIsNotNone(result)
        self.assertEqual(result.TypeId, "Mesh::Repair")
        self.assertIs(result.Source, source)
        self.assertTrue(result.RemoveDuplicates)
        self.assertFalse(result.HarmonizeNormals)
        self.assertFalse(result.RemoveNonManifolds)
        self.assertFalse(result.FixDegenerations)
        self.assertFalse(result.FixSelfIntersections)
        self.assertEqual(result.FillHolesMaxEdges, 0)
        self.assertEqual(source.Mesh.CountFacets, 5)
        self.assertEqual(result.Mesh.CountFacets, 4)
        self.assertFalse(source.Visibility)
        self.assertTrue(result.Visibility)
        ...
        self.assertTrue(result.Suppressed)
        self.assertEqual(result.Mesh.CountFacets, 5)
        ...
        with TemporaryDirectory() as temporary_directory:
            path = Path(temporary_directory) / "mesh-repair-service.FCStd"
            ...
            self.assertEqual(reopened_result.TypeId, "Mesh::Repair")
            self.assertIs(reopened_result.Source, reopened_source)
            self.assertTrue(reopened_result.RemoveDuplicates)
            self.assertEqual(
            ...
            self.assertEqual(reopened_source.Mesh.CountFacets, 5)
            self.assertEqual(reopened_result.Mesh.CountFacets, 4)
            self.assertFalse(reopened_source.Visibility)
            self.assertTrue(reopened_result.Visibility)
            ...
            self._timeline_button("VibeCADFeatureTimelinePrevious").click()
            self._process_events(10)
            self.assertTrue(reopened_result.Suppressed)
            self.assertEqual(reopened_result.Mesh.CountFacets, 5)
            self.assertFalse(reopened_result.Visibility)
            self.assertTrue(reopened_source.Visibility)

            self._timeline_button("VibeCADFeatureTimelineEnd").click()
            self._process_events(10)
            self.assertFalse(reopened_result.Suppressed)
            self.assertEqual(reopened_result.Mesh.CountFacets, 4)
            self.assertTrue(reopened_result.Visibility)
            self.assertFalse(reopened_source.Visibility)
            ...
            self.assertNotIn(
                ...
                [
                    operation.Name
                    for operation in reopened.getObject("VibeCADTimeline").Operations
                ],
            )
            ...
            self.assertIs(restored_result.Source, reopened_source)
            self.assertEqual(
                restored_result.VibeCADTimelineRole,
                "operation",
            )
            self.assertEqual(
                list(restored_result.VibeCADTimelineReplacedInputs),
                [reopened_source],
            )
            self.assertEqual(restored_result.Mesh.CountFacets, 4)
            self.assertFalse(restored_result.Suppressed)
            self.assertTrue(restored_result.Visibility)
            self.assertFalse(reopened_source.Visibility)
            self.assertIn(
                restored_result,
                reopened.getObject("VibeCADTimeline").Operations,
            )
    ...
    def test_mesh_repair_does_not_claim_an_initially_hidden_source(self):
        from tool_impl.service import mesh_repair
    ...
    def test_meshpart_conversion_services_are_source_preserving_operations(self):
        from tool_impl.service import meshpart_mesh_from_shape
        from tool_impl.service import meshpart_shape_from_mesh
        ...
        source_name = source.Name
        objects_before_mesh = tuple(self.document.Objects)
        undo_before_mesh = self.document.UndoCount

        mesh_response = meshpart_mesh_from_shape.run(
            service,
            source_object_name=source.Name,
            linear_deflection_mm=0.1,
            angular_deflection_degrees=28.5,
            label="Source-preserving mesh snapshot",
        )
        self.assertTrue(mesh_response["ok"], mesh_response)
        mesh_result = self.document.getObject(mesh_response["object"])
        self.assertIsNotNone(mesh_result)
        mesh_name = mesh_result.Name
        self.assertEqual(mesh_result.TypeId, "Mesh::Feature")
        self.assertGreater(mesh_result.Mesh.CountFacets, 0)
        self.assertTrue(source.Visibility)
        self.assertTrue(mesh_result.Visibility)
        self.assertNotIn(
            "VibeCADTimelineReplacedInputs",
            mesh_result.PropertiesList,
        )
        self.assertIn(
            mesh_result,
            self.document.getObject("VibeCADTimeline").Operations,
        )
        self.assertEqual(
        ...
        self.document.redo()
        self._process_events(8)
        mesh_result = self.document.getObject(mesh_name)
        self.assertIsNotNone(mesh_result)
        self.assertTrue(source.Visibility)
        self.assertTrue(mesh_result.Visibility)
        self.assertNotIn(
        ...
        objects_before_shape = tuple(self.document.Objects)
        undo_before_shape = self.document.UndoCount
        shape_response = meshpart_shape_from_mesh.run(
            service,
            mesh_object_name=mesh_result.Name,
            sewing_tolerance_mm=0.05,
            make_solid=False,
            label="Source-preserving faceted shape",
        )
        self.assertTrue(shape_response["ok"], shape_response)
        shape_result = self.document.getObject(shape_response["feature"])
        self.assertIsNotNone(shape_result)
        shape_name = shape_result.Name
        self.assertEqual(shape_result.TypeId, "Part::Feature")
        self.assertFalse(shape_result.Shape.isNull())
        self.assertTrue(mesh_result.Visibility)
        self.assertTrue(shape_result.Visibility)
        self.assertNotIn(
            "VibeCADTimelineReplacedInputs",
            shape_result.PropertiesList,
        )
        self.assertIn(
            shape_result,
            self.document.getObject("VibeCADTimeline").Operations,
        ...
        self.assertEqual(
            tuple(self.document.Objects),
            objects_before_shape,
        )
        self.assertTrue(mesh_result.Visibility)
        self.document.redo()
        self._process_events(8)
        mesh_result = self.document.getObject(mesh_name)
        shape_result = self.document.getObject(shape_name)
        self.assertIsNotNone(shape_result)
        self.assertTrue(mesh_result.Visibility)
        self.assertTrue(shape_result.Visibility)
        ...
        self._process_events(10)
        self.assertTrue(mesh_result.Visibility)
        self.assertFalse(shape_result.Visibility)
        self.assertTrue(source.Visibility)
        ...
        with TemporaryDirectory() as temporary_directory:
            path = (
                ...
                / "source-preserving-meshpart-conversions.FCStd"
            )
            self.document.saveAs(str(path))
            document_name = self.document.Name
            App.closeDocument(document_name)
            ...
            self.document = reopened
            reopened_source = reopened.getObject(source_name)
            reopened_mesh = reopened.getObject(mesh_name)
            reopened_shape = reopened.getObject(shape_name)
            self.assertIsNotNone(reopened_source)
            self.assertIsNotNone(reopened_mesh)
            self.assertIsNotNone(reopened_shape)
            self.assertTrue(reopened_source.Visibility)
            self.assertTrue(reopened_mesh.Visibility)
            self.assertTrue(reopened_shape.Visibility)
            self.assertNotIn(
                "VibeCADTimelineReplacedInputs",
                reopened_mesh.PropertiesList,
            )
            self.assertNotIn(
                "VibeCADTimelineReplacedInputs",
                reopened_shape.PropertiesList,
            )
