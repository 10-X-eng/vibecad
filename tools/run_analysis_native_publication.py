...
"""Run the verified Native publication transaction gate in an installed host."""
...
MARKER = "VIBECAD_ANALYSIS_NATIVE_PUBLICATION_OK "
...
INTEGRATION = (
    ...
    / "VibeCAD"
    / "vibecad_tests"
    / "analysis_native_publication_integration.py"
)
...
def parse_report(output: str) -> dict:
    ...
    if marker_index < 0:
        raise RuntimeError("The installed Native publication success marker is missing.")
    start = marker_index + len(MARKER)
    report, _end = json.JSONDecoder().raw_decode(output[start:].lstrip())
    if (
        not isinstance(report, dict)
        or report.get("runtime") != "installed-freecadcmd"
        or report.get("committed") is not True
        or report.get("rollback") is not True
    ):
        raise RuntimeError("The installed Native publication report is malformed.")
    return report
...
def run(executable: Path) -> dict:
    ...
    if completed.returncode:
        raise RuntimeError(
            f"Installed Native publication gate exited {completed.returncode}.\n{output}"
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("Installed verified Native publication transaction passed")
    print(f"Host: {executable}")
    print(
        "Scope: exact Document.Uid rebind plus real transaction commit, "
        "postcondition, rollback, and reopen persistence; fixture domain only."
    )
    return 0
